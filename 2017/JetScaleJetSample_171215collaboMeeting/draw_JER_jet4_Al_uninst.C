...
void draw_JER_jet4_Al_uninst(string ihcalType="Al_uninst",string jetE="20GeV", double fitmin=0.2, double fitmax=1.5, int ver=3, int patch=0, bool orig=true){
  ...
  TH2D* h01;
  if (orig) {h01 = (TH2D*)f01->Get("h1D_JER");}
  else { h01 = (TH2D*)f01->Get("h1D_modi_JER");}
  ...
  //c01->SaveAs(Form("outJet4JER/%s_%s_JER_ByGammaJet_%.0f_%.0f_%.0f.pdf",jetE.c_str(),ihcalType.c_str(),cemc_sf*100,ihcal_sf*100,ohcal_sf*100));
  c01->SaveAs(Form("outJet4JER/%s_%s_JER_ByGammaJet_ver_%d_patch_%d_orig%d.pdf",jetE.c_str(),ihcalType.c_str(),ver,patch,(int)orig));

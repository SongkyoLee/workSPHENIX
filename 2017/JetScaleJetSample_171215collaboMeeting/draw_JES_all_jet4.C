...
void draw_JES_all_jet4(string ihcalType="SS310",string jetE="50GeV", double fitmin=0.2, double fitmax=1.5, float cemc_sf=1.00, float ihcal_sf=1.00, float ohcal_sf=1.00){
  ...
  TH2D* h01 = (TH2D*)f01->Get("h1D_modi_JES");
  TH2D* h02 = (TH2D*)f01->Get("h1D_quark_modi_JES");
  TH2D* h03 = (TH2D*)f01->Get("h1D_gluon_modi_JES");
  h01->Sumw2();
  h02->Sumw2();
  h03->Sumw2();
...
//  gaus01->SetLineColor(kRed+1);
  ...
  //h01->Draw("pe");
  h02->GetXaxis()->SetTitle("E_{Tower jet} / E_{Truth jet}");
  ...
  c01->SaveAs(Form("outJet4JES/%s_%s_JES_all_ByGammaJet_%.0f_%.0f_%.0f.pdf",jetE.c_str(),ihcalType.c_str(),cemc_sf*100,ihcal_sf*100,ohcal_sf*100));

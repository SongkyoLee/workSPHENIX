...
void rootAna_emfrac_fromPion(TString ihcalType = "Jet_SS25Scale",int initfile=0,int endfile =50)
{
...
//  TH1D* h1D_towerjet4_cemc_e = new TH1D("towerjet4_cemc_e",";Tower Jet cemc E;",100,0,50);
  ...
  for(int j=initfile; j<endfile; j++){
    ...
    TString fname = Form("./out_%s/DVP_QCD35_10k_G4sPHENIX_OnlyHcalScaled_%d.root",ihcalType.Data(),j);
    if (j%10==0) cout << fname << endl; 
  ...
  TFile* fout = new TFile(Form("outEMfracFromPion/emfrac_%s.root",ihcalType.Data()),"RECREATE");
  fout->cd();

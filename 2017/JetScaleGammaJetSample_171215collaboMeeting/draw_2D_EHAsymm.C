...
float draw_2D_EHAsymm(string ihcalType = "SS310",
                    string jetE = "50GeV",
                    string inDir = "outJet4Matched",
                    string outDir = "outJet4_2DEHAsymm",
                    int initfile=0,
                    int endfile =1000,
                    float c_tot=1.00,
                    float c_em=1.00,
...
//  float io_max=0.8;
  ...
  if(f_EHAsymm){
    ...
    latex->DrawLatex(0.20,0.82,Form("p0: %.3f #pm %.3f",f_EHAsymm->GetParameter(0),f_EHAsymm->GetParError(0)));
    latex->DrawLatex(0.20,0.76,Form("p1: %.3f #pm %.3f",f_EHAsymm->GetParameter(1),f_EHAsymm->GetParError(1)));
  }

  c_EHAsymm->SaveAs(Form("./%s%s/%s_%s_totE_EHAsymm_scale_%.0f_%.0f_%.0f.pdf",outDir.c_str(),jetE.c_str(),jetE.c_str(),ihcalType.c_str(),cemc_sf*100,ihcal_sf*100,ohcal_sf*100));

...
float draw_2D_JES_emfrac(string ihcalType = "SS310",
                    string jetE = "50GeV",
                    ...
                    float c_ih=1.00)
{
  ...
  //float fitmin=-0.5;
  //float fitmax=0.5;
  TH2D* h2D_emfrac = (TH2D*)fin->Get("h2D_jet_JES_emfrac");
  ...
  c_emfrac->SaveAs(Form("./%s%s/%s_%s_JES_emfrac_scale_%.0f_%.0f_%.0f.pdf",outDir.c_str(),jetE.c_str(),jetE.c_str(),ihcalType.c_str(),cemc_sf*100,ihcal_sf*100,ohcal_sf*100));

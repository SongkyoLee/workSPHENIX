...
void draw_2D_EHAsymm_trial_v2(string outDir = "out2D_EHAsymm_Trial_v2",
                           string inType = "SS310_25GeV", 
                           bool do_cemcScale=false,
                           bool do_ihcalScale=true,
                           bool do_ohcalScale=true,
                           ...
                           float c_prim = 1.60)
{
  ...
  if (inType.compare("SS310_25GeV")==0){ 
    ...
    if (do_ihcalScale) c_ih=1.81;
    //if (do_ohcalScale) c_prim=1.3;
  }
    ...
    ptlType = "#pi^{#pm}";
    if (do_cemcScale) cemc_sf=1.;
    if (do_ihcalScale) c_ih=5.74;
    //if (do_ohcalScale) c_prim=1.3;
  }
  else {
    cout<<"WARNING! select correct inType" <<endl; return;
  } 
  ihcal_sf=c_ih*c_prim;
  ohcal_sf=c_prim;
  ...
  tr01->Draw(Form("(cemc_E3x3*%2f+ihcal_E3x3*%2f+ohcal_E3x3*%2f):((cemc_E3x3*%2f-(ihcal_E3x3*%2f+ohcal_E3x3*%2f))/(cemc_E3x3*%2f+ihcal_E3x3*%2f+ohcal_E3x3*%2f))>>h2d01",cemc_sf,ihcal_sf,ohcal_sf,cemc_sf,ihcal_sf,ohcal_sf,cemc_sf,ihcal_sf,ohcal_sf),Form("cemc_E3x3<%2f && ihcal_E3x3<%2f && (cemc_E3x3+ihcal_E3x3+ohcal_E3x3)>%2f",cemcCut,ihcalCut,totCut));
  tr02->Draw(Form("(cemc_E3x3*%2f+ihcal_E3x3*%2f+ohcal_E3x3*%2f):((cemc_E3x3*%2f-(ihcal_E3x3*%2f+ohcal_E3x3*%2f))/(cemc_E3x3*%2f+ihcal_E3x3*%2f+ohcal_E3x3*%2f))>>+h2d01",cemc_sf,ihcal_sf,ohcal_sf,cemc_sf,ihcal_sf,ohcal_sf,cemc_sf,ihcal_sf,ohcal_sf),Form("cemc_E3x3<%2f && ihcal_E3x3<%2f && (cemc_E3x3+ihcal_E3x3+ohcal_E3x3)>%2f",cemcCut,ihcalCut,totCut));
  ...
  c01->SaveAs(Form("%s/%s_2D_TOTE_vs_EHAsymm_%.0f_%.0f_%.0f_mipcut_%.0f_%.0f_totcut%.0f.pdf",outDir.c_str(),inType.c_str(),(int)do_cemcScale*cemc_sf*100,(int)do_ihcalScale*ihcal_sf*100,(int)do_ohcalScale*ohcal_sf*100,(int)do_cemcMipCut*cemcCut*100,(int)do_ihcalMipCut*ihcalCut*100,(int)do_totMipCut*totCut*100));

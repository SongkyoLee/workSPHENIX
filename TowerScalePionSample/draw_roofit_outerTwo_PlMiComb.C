...
void draw_roofit_outerTwo_PlMiComb(string inType = "SS310_25GeV", bool do_doubleGaus = true, bool ohcalScaled =false)
{
  float true_p;
  string ihcalType;
  string ptlType;
  ...
  if (true_p ==25) {
    if (ohcalScaled) {
      if (do_doubleGaus) {xmax=2.0; fitmin = 0.4; fitmax = 1.6; }
      else {xmax=2.0; fitmin = 0.4; fitmax = 1.6; } //to be determined
    } else {
      if (do_doubleGaus) { xmax=1.6; fitmin = 0.35; fitmax = 1.25; }
      else { xmax=1.6; fitmin = 0.4; fitmax = 0.95; }
    }
  } else if (true_p ==10) {
    if (ohcalScaled) {
      if (do_doubleGaus) {xmax=2.0; fitmin = 0.4; fitmax = 1.6; }
      ...
      if (do_doubleGaus) { xmax=1.6; fitmin = 0.3; fitmax = 1.2; }
      else { xmax=1.6; fitmin = 0.35; fitmax = 0.9; }
    }
  }

  TFile* fin01 = new TFile(Form("out_scale/%s_PiPl_%.0fGeV_scale_0_0_%d.root",ihcalType.c_str(),true_p,(int)ohcalScaled),"READ");
  TFile* fin02 = new TFile(Form("out_scale/%s_PiMi_%.0fGeV_scale_0_0_%d.root",ihcalType.c_str(),true_p,(int)ohcalScaled),"READ");
  TH1D* h01;
  TH1D* h02;
  if (ohcalScaled){
    h01 = (TH1D*)fin01->Get("cemcMipCut/outerTwo_frac_E3x3_cut1");
    h02 = (TH1D*)fin02->Get("cemcMipCut/outerTwo_frac_E3x3_cut1");
  ...
  c1->SaveAs(Form("out_scale/%s_outerTwo_doubleGaus%d_ohcalScaled%d.pdf",inType.c_str(),(int)do_doubleGaus,(int)ohcalScaled));
  return; 
}

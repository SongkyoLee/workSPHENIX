...
void draw_2D_E5x5_EHAsymm(string outDir = "out2D_E5x5_EHAsymm",
                           ...
                           bool do_profile=true)
{
  ...
  int nbin=400;  
  TH2D* h2d01 = new TH2D("h2d01",";(E_{CEMC} - E_{IHCAL+OHCAL})/(E_{CEMC} + E_{IHCAL+OHCAL});E_{CEMC} + E_{IHCAL} + E_{OHCAL}",nbin,-1,1,nbin,ymin,ymax); 
 
  //// fill the histo!
  TTree* tr01 = (TTree*)fin01->Get("track_tree");
  TTree* tr02 = (TTree*)fin02->Get("track_tree");
  tr01->Draw(Form("(cemc_E5x5*%2f+ihcal_E5x5*%2f+ohcal_E5x5*%2f):((cemc_E5x5*%2f-(ihcal_E5x5*%2f+ohcal_E5x5*%2f))/(cemc_E5x5*%2f+ihcal_E5x5*%2f+ohcal_E5x5*%2f))>>h2d01",cemc_sf,ihcal_sf,ohcal_sf,cemc_sf,ihcal_sf,ohcal_sf,cemc_sf,ihcal_sf,ohcal_sf),Form("cemc_E5x5<%2f && ihcal_E5x5<%2f && (cemc_E5x5+ihcal_E5x5+ohcal_E5x5)>%2f",cemcCut,ihcalCut,totCut));
  tr02->Draw(Form("(cemc_E5x5*%2f+ihcal_E5x5*%2f+ohcal_E5x5*%2f):((cemc_E5x5*%2f-(ihcal_E5x5*%2f+ohcal_E5x5*%2f))/(cemc_E5x5*%2f+ihcal_E5x5*%2f+ohcal_E5x5*%2f))>>+h2d01",cemc_sf,ihcal_sf,ohcal_sf,cemc_sf,ihcal_sf,ohcal_sf,cemc_sf,ihcal_sf,ohcal_sf),Form("cemc_E5x5<%2f && ihcal_E5x5<%2f && (cemc_E5x5+ihcal_E5x5+ohcal_E5x5)>%2f",cemcCut,ihcalCut,totCut));
  ...
  c01->SaveAs(Form("%s/%s_2D_E5x5_TOTE_vs_EHAsymm_%.0f_%.0f_%.0f_mipcut_%.0f_%.0f_totcut%.0f.pdf",outDir.c_str(),inType.c_str(),(int)do_cemcScale*c2nd*100,(int)do_ihcalScale*c1st*100,(int)do_totalScale*c3rd*100,(int)do_cemcMipCut*cemcCut*100,(int)do_ihcalMipCut*ihcalCut*100,(int)do_totMipCut*totCut*100));

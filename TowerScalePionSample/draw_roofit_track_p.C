...
void draw_roofit_track_p(string outDir = "outScaleTrackP", string inType = "SS310_25GeV", bool do_doubleGaus = true)
{
  float true_p;
  string ihcalType;
  string ptlType;
  ...
  float fitmin, fitmax;
  if (true_p ==25) {
    if (do_doubleGaus) { xmin=20.; xmax=30.; fitmin = 22.; fitmax = 28.; }
    else { xmin=20.; xmax=30.; fitmin = 22.; fitmax = 28.; }
  } else if (true_p ==10) {
    if (do_doubleGaus) { xmin=9.; xmax=11.; fitmin = 9.5; fitmax = 10.5; }
    else { xmin=9.; xmax=11.; fitmin = 9.5; fitmax = 10.5; }
  }

  TFile* fin01 = new TFile(Form("%s/%s_PiPl_%.0fGeV_scale_0_0_0.root",outDir.c_str(),ihcalType.c_str(),true_p),"READ");
  TFile* fin02 = new TFile(Form("%s/%s_PiMi_%.0fGeV_scale_0_0_0.root",outDir.c_str(),ihcalType.c_str(),true_p),"READ");
  TH1D* h01;
  TH1D* h02;
  h01 = (TH1D*)fin01->Get("track_p");
  h02 = (TH1D*)fin02->Get("track_p");
  cout << "h01: " << h01 << endl;
  cout << "h02: " << h02 << endl;
  h01->Add(h02);
  //h01->Rebin(4);
  ...
  RooRealVar* mean = new RooRealVar("mean","mean",true_p,xmin,xmax) ;
  //RooRealVar* sigma01= new RooRealVar("sigma01","sigma01",0.2*true_p,0.1,1.5*true_p) ;
  //RooRealVar* sigma02= new RooRealVar("sigma02","sigma02",0.7*true_p,0.2,2.0*true_p) ;
  RooRealVar* sigma01= new RooRealVar("sigma01","sigma01",0.2,0.01,0.7) ;
  RooRealVar* sigma02= new RooRealVar("sigma02","sigma02",0.7,0.01,1.2) ;
  ...
  if (do_doubleGaus){
    latex->DrawLatex(0.90,0.80,Form("#mu: %.1f #pm %.1f", mean->getVal(),mean->getError()));
    latex->DrawLatex(0.90,0.75,Form("#sigma: %.1f #pm %.1f", sigmaTot,sigmaTotErr));
    latex->DrawLatex(0.90,0.70,Form("#sigma/#mu: %.3f", sigmaTot/mean->getVal()));
  } else {
    latex->DrawLatex(0.90,0.80,Form("#mu: %.1f #pm %.1f", mean->getVal(),mean->getError()));
    latex->DrawLatex(0.90,0.75,Form("#sigma: %.1f #pm %.1f", sigma01->getVal(),sigma01->getError()));
    latex->DrawLatex(0.90,0.70,Form("#sigma/#mu: %.3f", sigma01->getVal()/mean->getVal()));
  }
  latex->Draw();
  
  c1->SaveAs(Form("%s/%s_track_p_doubleGaus%d.pdf",outDir.c_str(),inType.c_str(),(int)do_doubleGaus));
  return; 
}

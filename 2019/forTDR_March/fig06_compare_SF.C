void fig06_compare_SF(string ihcalType="Alframe"){
  ...
  TLegend* leg1;
  if (ihcalType.compare("SS310")==0) {leg1 = new TLegend(0.20, 0.45, 0.42, 0.55);}
  else if (ihcalType.compare("Alframe")==0) {leg1 = new TLegend(0.20, 0.45, 0.42, 0.55);}
  else { cout << "WRONG!"<<endl; return;}
  ...
  leg3->AddEntry(gohcal_2," C");  
  if (ihcalType.compare("SS310")==0) leg2->Draw();
  else { leg3->Draw(); }
  c_sf->SaveAs(Form("out_figs/hcal_SFvsE_%s.pdf",ihcalType.c_str()));

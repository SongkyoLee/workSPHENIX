...
void rootAna_ohcalScale(string inType = "SS310_PiPl_25GeV",int initfile=0,int endfile =10,float cemc_cut=1.0, float ihcal_cut=1.0)
{
  bool do_E3x3 = true;
  bool do_E5x5 = true;
  bool do_clE = true;
  ...
  for(int isim=0; isim<nsim; isim++){
    ...
    for(int itrk=0; itrk<track_n; itrk++){
      ...
      if (do_E5x5){    
        cemc_e=cemc_E5x5[itrk];
        ihcal_e=ihcal_E5x5[itrk];
        ohcal_e=ohcal_E5x5[itrk];
      ...
      if (do_clE){    
        cemc_e=cemc_clE[itrk];
        ihcal_e=ihcal_clE[itrk];
        ohcal_e=ohcal_clE[itrk];

...
void draw_roofit_ohcal(string inType = "SS310_PiPl_25GeV", bool do_doubleGaus = true, bool ohcalScaled =false)
{
  float true_p;
  string ihcalType;
  string ptlType;
  ...
  TFile* fin = new TFile(Form("out_scale/%s_scale_0_0_%d.root",inType.c_str(),(int)ohcalScaled),"READ");
  cout << "fin: " << fin << endl;
  ...
  fracframe->GetXaxis()->SetTitle("(E_{IHCAL} + E_{OHCAL}) / (E_{Truth} - E_{CEMC})");
  ...
  c1->SaveAs(Form("out_scale/%s_ohcal_doubleGaus%d_ohcalScaled.pdf",inType.c_str(),(int)do_doubleGaus,(int)ohcalScaled));
  return; 
}

...
void rootAna_GluonQuarkJet4_Al_uninst_test(string ihcalType = "Al_uninst",string jetE="50GeV",int initfile=0,int endfile =1, int ver = 3, int patch =0)
{
  ...
  TFile* filefit = new TFile(Form("../JetScaleGammaJetSample/outJet4_2DEHAsymm_Al_uninst/%s_Al_uninst_totE_EHAsymm_scale_%.0f_%.0f_%.0f.root",jetE.c_str(),cemc_sf*100,ihcal_sf*100,ohcal_sf*100),"READ");
  ...
  //// test
  TH2D* h2D_EHAsymm_emfrac = new TH2D("h2D_EHAsymm_emfrac",";Truth Jet EM fraction;E_{CEMC}-E_{HCAL} Asymm",100, 0., 1., 100, -1.0, 1.0);
  TH2D* h2D_modi_EHAsymm_emfrac = new TH2D("h2D_modi_EHAsymm_emfrac",";Truth Jet EM fraction;E_{CEMC}-E_{HCAL} Asymm",100, 0., 1., 100, -1.0, 1.0);
  ////////////////////////////////////////////////////////
  ////////////////////////////////////////////////////////
  ////////////////////////////////////////////////////////
  //// loop over files 
...
//    float truthjet4_cons_pid[999];  // Al,, KYO: to be modified!!!!!
    ...
    for (int itrue=0; itrue<6; itrue++){
      ...
      //// for Al_uninst
      float reco_jet_EHAsymm_tmp = (cemc_sum-ihcal_sum-ohcal_sum)/(cemc_sum+ihcal_sum+ohcal_sum);
      if (ver==1) {
        modi_cemc_sum=cemc_sum;
        modi_ihcal_sum=ihcal_sum;
        modi_ohcal_sum=ohcal_sum*refval/f_EHAsymm->Eval(reco_jet_EHAsymm_tmp);
      } 
      else if (ver==2){ //wrong
        modi_cemc_sum=cemc_sum;
        modi_ihcal_sum=ihcal_sum;
        modi_ohcal_sum=( ohcal_sum*refval/f_EHAsymm->Eval(reco_jet_EHAsymm_tmp) ) * ( 2./(1-reco_jet_EHAsymm_tmp) );
      }
      else if (ver==3){
        modi_cemc_sum=cemc_sum*refval/f_EHAsymm->Eval(reco_jet_EHAsymm_tmp);
        modi_ihcal_sum=ihcal_sum*refval/f_EHAsymm->Eval(reco_jet_EHAsymm_tmp);
        modi_ohcal_sum=ohcal_sum*refval/f_EHAsymm->Eval(reco_jet_EHAsymm_tmp);
      }
      else if (ver==4) {
        if (patch==0) {
          modi_cemc_sum=cemc_sum;
          modi_ihcal_sum=ihcal_sum;
          modi_ohcal_sum=ohcal_sum*
                ( 1+ (refval-f_EHAsymm->Eval(reco_jet_EHAsymm_tmp))/(ohcal_sum/truthjet4_e) ); //// truth info..
        } else if (patch==1){
          modi_cemc_sum=cemc_sum;
          modi_ihcal_sum=ihcal_sum;
          modi_ohcal_sum=ohcal_sum*
                ( 1+ (refval-f_EHAsymm->Eval(reco_jet_EHAsymm_tmp))/(ohcal_sum/(cemc_sum+ihcal_sum+ohcal_sum)) ); //reco info
        }
      } 
      else if (ver==5) {
        modi_cemc_sum=cemc_sum;
      ...
      float reco_jet_EHAsymm = (modi_cemc_sum-modi_ihcal_sum-modi_ohcal_sum)/(modi_cemc_sum+modi_ihcal_sum+modi_ohcal_sum);
      ...
      cout << "" << endl; 
      cout << "em_frac = " << em_frac << endl;
      cout << "f_EHAsymm->Eval(reco_jet_EHAsymm_tmp) = "<<f_EHAsymm->Eval(reco_jet_EHAsymm_tmp) << endl;
      cout << "cemc_sum = " << cemc_sum << endl;
      cout << "ihcal_sum = " << ihcal_sum << endl;
      cout << "ohcal_sum = " << ohcal_sum << endl;
      cout << "reco_jet_EHAsymm_tmp = "<< reco_jet_EHAsymm_tmp << endl;
      cout << "modi_cemc_sum = " << modi_cemc_sum << endl;
      cout << "modi_ihcal_sum = " << modi_ihcal_sum << endl;
      cout << "modi_ohcal_sum = " << modi_ohcal_sum << endl;
      cout << "reco_jet_EHAsymm = "<< reco_jet_EHAsymm << endl;
  
      h2D_EHAsymm_emfrac->Fill(em_frac,reco_jet_EHAsymm_tmp);
      h2D_modi_EHAsymm_emfrac->Fill(em_frac,reco_jet_EHAsymm);
  ...
  h2D_EHAsymm_emfrac->SetOption("colz");
  h2D_modi_EHAsymm_emfrac->SetOption("colz");
 
  /* 
  TFile* fout = new TFile(Form("outJet4QGAlUninstTEST/%s_%s_ByGammaJet_ver_%d_patch_%d.root",jetE.c_str(),ihcalType.c_str(),ver,patch),"RECREATE");
  fout->cd();
  ////
  ...
  h2D_EHAsymm_emfrac->Write();
  h2D_modi_EHAsymm_emfrac->Write();

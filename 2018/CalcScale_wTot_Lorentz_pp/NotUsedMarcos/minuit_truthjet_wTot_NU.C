...
void minuit_truthjet_wTot(string sampleType = "QCDJet",
...
{  
  ...
  static Double_t vstart[4] = {1.0, 1.0, 1.0, 1.0}; //staring values
  static Double_t step[4] = {0.01, 0.01, 0.01, 0.01}; //step
  ...
  gMinuit->mnparm(3, "total_scale", vstart[3], step[3], 1,10,ierflg);
  ...
  gMinuit->GetParameter(3,totalScale,totalScaleErr);
  ...
  for (int ievt=0; ievt <nevt; ievt++){
    ...
    reco_jet_caliE = totalScale*(reco_jet_clcemc_emEsum + cemcScale*reco_jet_clcemc_hadEsum + ihcalScale*reco_jet_ihcalEsum + ohcalScale*reco_jet_ohcalEsum);
  ...
  c1->SaveAs(Form("minuit_truthjet_wTot/%d_%d_%s_%s_%s_%s.pdf",initfile,endfile,sampleType.c_str(),jetE.c_str(),ihcalType.c_str(),note.c_str()));
  ...
  c2->SaveAs(Form("minuit_truthjet_wTot/%d_%d_%s_%s_%s_%s_v2.pdf",initfile,endfile,sampleType.c_str(),jetE.c_str(),ihcalType.c_str(),note.c_str()));

  cout << "cemc_tmp = " << cemcScale << endl;
  cout << "ihcal_tmp = " << ihcalScale << endl;
  cout << "ohcal_tmp = " << ohcalScale << endl;
  cout << "total_tmp = " << totalScale << endl;
  cout << "" << endl; 
  cout << "cemcScale = " << cemcScale*totalScale << endl;
  cout << "ihcalScale = " << ihcalScale*totalScale << endl;
  cout << "ohcalScale = " << ohcalScale*totalScale << endl;
  cout << "cemcEmScale = " << 1.*totalScale << endl;
...
Double_t func(Double_t x1, Double_t x2, Double_t x3, Double_t x4, Double_t *par){
  ...
  return par[3]*(x1 + par[0]*x2 + par[1]*x3 + par[2]*x4); //SS310wTot
}

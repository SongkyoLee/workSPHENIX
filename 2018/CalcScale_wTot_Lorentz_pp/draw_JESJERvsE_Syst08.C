void draw_JESJERvsE_Syst08(string ihcalType="SS310", string note="R04"){
  ...
  for (int im=0; im<nMeth; im++){
    infile_jesAfter[im].open(Form("txt_jet_syst08/g/jes_%s_%s_%s.out",szMeth[im].c_str(),ihcalType.c_str(),note.c_str()));
    ijes=0;
  ...
  for (int im=0; im<nMeth; im++){
    infile_jerAfter[im].open(Form("txt_jet_syst08/g/jer_%s_%s_%s.out",szMeth[im].c_str(),ihcalType.c_str(),note.c_str()));
    ijer=0;

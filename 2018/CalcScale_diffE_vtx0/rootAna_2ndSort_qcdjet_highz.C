...
void rootAna_2ndSort_qcdjet_highz(string sampleType = "QCDJet",
                              string ihcalType = "Alframe",
                              string jetInE = "40GeV",
                              string jetOutE = "40GeV",
                              string vtxType = "highz",
                              int initfile=0,
                              int endfile =1000,
                              bool doetopcut=true,
                              bool dodphicut=true)
{
  ...
  string foutname  = Form("./2ndSortedRootFiles/2ndSorted_%dto%d_%s_G4sPHENIX_jet4_%s_%s_doetopcut%d_dodphicut%d_%s.root",initfile,endfile,sampleType.c_str(),jetOutE.c_str(),ihcalType.c_str(),(int)doetopcut,(int)dodphicut,vtxType.c_str()); 
  //string foutname = Form("./2ndSortedRootFiles/2ndSorted_%dto%d_%s_G4sPHENIX_jet4_%s_%s_doetopcut%d_dodphicut%d_highz_noemcut.root",initfile,endfile,sampleType.c_str(),jetOutE.c_str(),ihcalType.c_str(),(int)doetopcut,(int)dodphicut);
  ...
  //for (int ievt=0; ievt<50; ievt++){ 
    ...
    for (int itrue=0; itrue<true_n; itrue++){
      ...
      if (vtxType.compare("highz")==0 || vtxType.compare("noptcut_highz")==0 ) {
        if (true_lcons_z[itrue]<0.8) continue;
        if (true_emfrac[itrue]>0.75) continue; //remove EM particles
      }

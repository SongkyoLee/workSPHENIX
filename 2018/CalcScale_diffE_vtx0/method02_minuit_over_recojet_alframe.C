...
void method02_minuit_over_recojet_alframe(string sampleType = "GammaJet",
...
{  
  ...
  //// before 
  TH1D* h_origRESRecoJet = new TH1D("h_origRESRecoJet",";(Jet E_{Reco} - #gamma E_{Reco}) / Jet E_{Reco};Counts",100,-1.5,1.5);
  TH2D* h_emfrac_origRESRecoJet = new TH2D("h_emfrac_origRESRecoJet",";Truth Jet EM fraction;(Jet E_{Reco} - #gamma E_{Reco}) / Jet E_{Reco}",101,-0.05,1.05,100,-1.5,1.5);
  ...
  //// after
  TH1D* h_RESRecoJet = new TH1D("h_RESRecoJet",";(Jet E_{Reco} - #gamma E_{Reco}) / Jet E_{Reco};Counts",100,-1.5,1.5);
  TH2D* h_emfrac_RESRecoJet = new TH2D("h_emfrac_RESRecoJet",";Truth Jet EM fraction;(Jet E_{Reco} - #gamma E_{Reco}) / Jet E_{Reco}",101,-0.05,1.05,100,-1.5,1.5);
  ...
  for (int ievt=0; ievt <nevt; ievt++){
    ...
    h_origRESRecoJet->Fill( (reco_jet_origE-reco_gamma_e)/reco_jet_origE );    
    h_emfrac_origRESRecoJet->Fill( true_jet_emfrac, (reco_jet_origE-reco_gamma_e)/reco_jet_origE );
    h_emfrac_origJESRecoGamma->Fill( true_jet_emfrac, reco_jet_origE/reco_gamma_e );
    h_emfrac_origJESTruthJet->Fill( true_jet_emfrac, reco_jet_origE/true_jet_e );

    gamma_E.push_back(reco_gamma_e); 
  ...
  gMinuit->mnparm(1, "ohcal_scale", vstart[1], step[1], 0,0,ierflg);
  ...
  for (int ievt=0; ievt <nevt; ievt++){
    ...
    h_RESRecoJet->Fill( (reco_jet_caliE-reco_gamma_e)/reco_jet_caliE );    
    h_emfrac_RESRecoJet->Fill( true_jet_emfrac, (reco_jet_caliE-reco_gamma_e)/reco_jet_caliE );
    h_emfrac_JESRecoGamma->Fill( true_jet_emfrac, reco_jet_caliE/reco_gamma_e );
    h_emfrac_JESTruthJet->Fill( true_jet_emfrac, reco_jet_caliE/true_jet_e );
  
  }

  SetHistStyle(h_origRESRecoJet,0,0); 
  h_emfrac_origRESRecoJet->SetOption("colz");
  h_emfrac_origJESRecoGamma->SetOption("colz");
  h_emfrac_origJESTruthJet->SetOption("colz");
  
  SetHistStyle(h_RESRecoJet,0,0); 
  h_emfrac_RESRecoJet->SetOption("colz");
  ...
  c1->Divide(2,2);
  c1->cd(1);
  h_origRESRecoJet->Draw("pe");
  c1->cd(2);
  h_RESRecoJet->Draw("pe");
  c1->cd(3);
  h_emfrac_origRESRecoJet->Draw();
  c1->cd(4);
  h_emfrac_RESRecoJet->Draw();
  ...
  TProfile *prof_emfrac_origRESRecoJet = h_emfrac_origRESRecoJet->ProfileX();
  prof_emfrac_origRESRecoJet->Fit("pol1","","",fitmin4,fitmax4); 
  gStyle->SetOptFit(0);
  //gStyle->SetOptFit(1);
  prof_emfrac_origRESRecoJet->SetMarkerColor(kCyan+1);
  prof_emfrac_origRESRecoJet->SetLineColor(kCyan+1);
  prof_emfrac_origRESRecoJet->SetMarkerSize(1.);
  TF1 *f_emfrac_origRESRecoJet = (TF1*)prof_emfrac_origRESRecoJet->GetListOfFunctions()->FindObject("pol1");
  if(f_emfrac_origRESRecoJet){
    f_emfrac_origRESRecoJet->SetLineColor(kRed-4);
    f_emfrac_origRESRecoJet->SetLineWidth(4);
    f_emfrac_origRESRecoJet->SetRange(fitmin4,fitmax4);
    h_emfrac_origRESRecoJet->Draw();
    prof_emfrac_origRESRecoJet->Draw("ep same"); 
    
    latex->SetTextAlign(12);
    latex->SetTextSize(0.040);
    latex->DrawLatex(0.63,0.88,Form("#chi^{2}/ndf: %.2f/%d",f_emfrac_origRESRecoJet->GetChisquare(),f_emfrac_origRESRecoJet->GetNDF()));
    latex->DrawLatex(0.63,0.82,Form("p0: %.3f #pm %.3f",f_emfrac_origRESRecoJet->GetParameter(0),f_emfrac_origRESRecoJet->GetParError(0)));
    latex->DrawLatex(0.63,0.76,Form("p1: %.3f #pm %.3f",f_emfrac_origRESRecoJet->GetParameter(1),f_emfrac_origRESRecoJet->GetParError(1)));  
  }
  c1->Update(); //mandatory
  palette = (TPaletteAxis*)h_emfrac_origRESRecoJet->GetListOfFunctions()->FindObject("palette");
  ...
  TProfile *prof_emfrac_RESRecoJet = h_emfrac_RESRecoJet->ProfileX();
  prof_emfrac_RESRecoJet->Fit("pol1","","",fitmin4,fitmax4); 
  gStyle->SetOptFit(0);
  //gStyle->SetOptFit(1);
  prof_emfrac_RESRecoJet->SetMarkerColor(kCyan+1);
  prof_emfrac_RESRecoJet->SetLineColor(kCyan+1);
  prof_emfrac_RESRecoJet->SetMarkerSize(1.);
  TF1 *f_emfrac_RESRecoJet = (TF1*)prof_emfrac_RESRecoJet->GetListOfFunctions()->FindObject("pol1");
  if(f_emfrac_RESRecoJet){
    f_emfrac_RESRecoJet->SetLineColor(kRed-4);
    f_emfrac_RESRecoJet->SetLineWidth(4);
    f_emfrac_RESRecoJet->SetRange(fitmin4,fitmax4);
    h_emfrac_RESRecoJet->Draw();
    prof_emfrac_RESRecoJet->Draw("ep same"); 
    
    latex->SetTextAlign(12);
    latex->SetTextSize(0.040);
    latex->DrawLatex(0.63,0.88,Form("#chi^{2}/ndf: %.2f/%d",f_emfrac_RESRecoJet->GetChisquare(),f_emfrac_RESRecoJet->GetNDF()));
    latex->DrawLatex(0.63,0.82,Form("p0: %.3f #pm %.3f",f_emfrac_RESRecoJet->GetParameter(0),f_emfrac_RESRecoJet->GetParError(0)));
    latex->DrawLatex(0.63,0.76,Form("p1: %.3f #pm %.3f",f_emfrac_RESRecoJet->GetParameter(1),f_emfrac_RESRecoJet->GetParError(1)));  
  }
  c1->Update(); //mandatory
  palette = (TPaletteAxis*)h_emfrac_RESRecoJet->GetListOfFunctions()->FindObject("palette");
  x1ndc = palette->GetX1NDC();
  x2ndc = palette->GetX2NDC();
  palette->SetX1NDC(0.87);
  palette->SetX2NDC(0.92);
  
  c1->SaveAs(Form("method02/over_recojet_%d_%d_%s_%s_%s.pdf",initfile,endfile,sampleType.c_str(),jetE.c_str(),ihcalType.c_str()));
  ...
  c2->SaveAs(Form("method02/over_recojet_%d_%d_%s_%s_%s_v2.pdf",initfile,endfile,sampleType.c_str(),jetE.c_str(),ihcalType.c_str()));

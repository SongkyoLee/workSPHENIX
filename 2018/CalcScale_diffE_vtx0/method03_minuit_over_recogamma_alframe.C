...
void method03_minuit_over_recogamma_alframe(string sampleType = "GammaJet",
...
{  
  ...
  for (int ievt=0; ievt <nevt; ievt++){
    ...
    h_origRESRecoGamma->Fill( (reco_jet_origE-reco_gamma_e)/reco_gamma_e );    
    h_emfrac_origRESRecoGamma->Fill( true_jet_emfrac, (reco_jet_origE-reco_gamma_e)/reco_gamma_e );
  ...
  //static Double_t vstart[2] = {1.0, 1.0, 1.0}; //staring values
  //static Double_t step[2] = {0.01, 0.01, 0.01}; //step
  ...
  for (int ievt=0; ievt <nevt; ievt++){
    ...
    h_RESRecoGamma->Fill( (reco_jet_caliE-reco_gamma_e)/reco_gamma_e );    
    h_emfrac_RESRecoGamma->Fill( true_jet_emfrac, (reco_jet_caliE-reco_gamma_e)/reco_gamma_e );
  ...
  c1->SaveAs(Form("method03/over_recogamma_%d_%d_%s_%s_%s_v2.pdf",initfile,endfile,sampleType.c_str(),jetE.c_str(),ihcalType.c_str()));

...
//  palette->SetX2NDC(1-margin*3);
  ...
  ////////////////////////////////////////
  if (total_sf !=1) {
    c1->SaveAs(Form("outFinal%s/final2D_%s_%s_%s_%s_%.0f_%.0f_%.0f_wTot%.0f.pdf",ihcalType.c_str(),sampleType.c_str(),jetE.c_str(),ihcalType.c_str(),note.c_str(),100*cemc_sf,100*ihcal_sf,100*ohcal_sf,100*total_sf));
  } else {
  ...
  if (total_sf !=1) {
    c2->SaveAs(Form("outFinal%s/final1D_%s_%s_%s_%s_%.0f_%.0f_%.0f_wTot%.0f.pdf",ihcalType.c_str(),sampleType.c_str(),jetE.c_str(),ihcalType.c_str(),note.c_str(),100*cemc_sf,100*ihcal_sf,100*ohcal_sf,100*total_sf));
  } else {
  ...
  if (cemc_sf==1 && ihcal_sf==1 && ohcal_sf==1) { 
    fout = new TFile(Form("outFinal%s/final_%s_%s_%s_%s_before.root",ihcalType.c_str(),sampleType.c_str(),jetE.c_str(),ihcalType.c_str(),note.c_str()),"RECREATE"); 
  } else { 
    if (total_sf!=1){
      fout = new TFile(Form("outFinal%s/final_%s_%s_%s_%s_truthjet_wTot.root",ihcalType.c_str(),sampleType.c_str(),jetE.c_str(),ihcalType.c_str(),note.c_str()),"RECREATE"); //tmp 
    } else {
      fout = new TFile(Form("outFinal%s/final_%s_%s_%s_%s_truthjet.root",ihcalType.c_str(),sampleType.c_str(),jetE.c_str(),ihcalType.c_str(),note.c_str()),"RECREATE"); //tmp
    }   
  }

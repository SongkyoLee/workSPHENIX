void draw_JESJERvsE_Syst01(string ihcalType="Al", string note="04fmR04"){
  ...
  for (int im=0; im<nMeth; im++){
    infile_jesAfter[im].open(Form("txt_jet_syst01/jes_%s_%s_%s.out",szMeth[im].c_str(),ihcalType.c_str(),note.c_str()));
    ijes=0;
  ...
  c_JES->SaveAs(Form("outvsE_Syst01/JESvsE_%s_%s.pdf",ihcalType.c_str(),note.c_str()));
  ///////////////////////////////////////
  ...
  for (int im=0; im<nMeth; im++){
    infile_jerAfter[im].open(Form("txt_jet_syst01/jer_%s_%s_%s.out",szMeth[im].c_str(),ihcalType.c_str(),note.c_str()));
    ijer=0;
  ...
  c_JER->SaveAs(Form("outvsE_Syst01/JERvsE_%s_%s.pdf",ihcalType.c_str(),note.c_str()));
  ///////////////////////////////////////
  
  TFile* fout =  new TFile(Form("outvsE_Syst01/JESvsE_%s_%s.root",ihcalType.c_str(),note.c_str()),"RECREATE");
  fout->cd();
  gjesBefore->Write();
  for (int im=0; im<nMeth; im++){   
    gjesAfter[im]->Write();

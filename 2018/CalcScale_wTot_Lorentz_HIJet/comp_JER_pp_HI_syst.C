void comp_JER_pp_HI_syst(string ihcalType="Al", string note1st="04fm", string note2nd="R04", int nTerm = 0){
  ...
  TLegend* leg1 = new TLegend(0.18, 0.18, 0.38, 0.33);
  //leg1->SetTextSize(0.045);
  leg1->SetTextSize(0.025);

  TFile* fin01 =  new TFile(Form("../z_20180912_wTot_pp_Lorentz/outvsE_Syst/JESvsE_%s_%s.root",ihcalType.c_str(),note2nd.c_str()),"READ");
  cout << fin01 << endl;
  if(!fin01->IsOpen()) { cout <<"WARNING!! fin01 not found"<<endl; return;};
  TFile* fin02 =  new TFile(Form("../z_20180912_wTot_HIJet_Lorentz/outvsE_Syst/JESvsE_%s_%s%s.root",ihcalType.c_str(),note1st.c_str(),note2nd.c_str()),"READ");
  ...
  TGraphAsymmErrors* pp_sys = (TGraphAsymmErrors*)fin01->Get("gjerSys");
  ...
  TGraphAsymmErrors* hi_sys = (TGraphAsymmErrors*)fin02->Get("gjerSys");

  SetSoftGraphStyle(pp_before,2,10);
  SetSoftGraphStyle(pp_sys,2,10);
  pp_sys->SetFillColorAlpha(kBlue-10,0.5);
  SetSoftGraphStyle(pp_after,2,0);
  SetSoftGraphStyle(hi_before,1,10);
  SetSoftGraphStyle(hi_sys,1,10);
  hi_sys->SetFillColorAlpha(kRed-10,0.5);
  SetSoftGraphStyle(hi_after,1,0);
  
  TCanvas* c1 = new TCanvas("c1","c1",600,600);
  if(note2nd.compare("R04")==0) pp_before->SetMaximum(0.58);
  else pp_before->SetMaximum(0.58);
  pp_before->Draw("ap"); 
  hi_before->Draw("p"); 
  pp_sys->Draw("5"); 
  hi_sys->Draw("5"); 
...
*/
  ...
  c1->SaveAs(Form("outPPHI/comp_syst_%s_%s_%s_nTerm%d.pdf",ihcalType.c_str(),note1st.c_str(),note2nd.c_str(),nTerm));  

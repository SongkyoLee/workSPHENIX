...
void rootAna_1stSort_py8jet_lorentz_AltoAlframe(string label = "py8JetallGeVR04_Al",
                            int initfile=0,
                            int endfile = 50, 
                            float total_sf = 1.00,
                            float cemc_sf = 1.05,
                            ...
                            bool dodphicut=true)
{
  ...
  string foutname = Form("./sortedRootFilesLorentzAltoAlframe/sorted_%dto%d_%s_G4sPHENIX_jet4_%s_%s_%s_doetopcut%d_dodphicut%d_doscale%d.root",initfile,endfile,sampleType.c_str(),jetE.c_str(),ihcalType.c_str(),note.c_str(),(int)doetopcut,(int)dodphicut,(int)doscale);
...
*/
      ...
      if (towerjet4_pt<30) { cemc_sf = cemc_diff[0]; ihcal_sf = ihcal_diff[0]; ohcal_sf = ohcal_diff[0]; }
      else if (towerjet4_pt>30 && towerjet4_pt<40) { cemc_sf = cemc_diff[1]; ihcal_sf = ihcal_diff[1]; ohcal_sf = ohcal_diff[1]; }
      else if (towerjet4_pt>40) { cemc_sf = cemc_diff[2]; ihcal_sf = ihcal_diff[2]; ohcal_sf = ohcal_diff[2]; }
...
//        tmp_calib_myjet += calib_ihcal.at(iihcal); //AltoAlframe
      }
      ...
      for (int icemc =0; icemc < towerjet4_clcemc_n; icemc ++) {
        ...
        if (isHad){
          ...
          tmp_calib_clcemc_had.SetPtEtaPhiM(total_sf*cemc_sf*towerjet4_clcemc_pt->at(icemc),towerjet4_clcemc_eta->at(icemc),towerjet4_clcemc_phi->at(icemc),0);
      ...
      for (int icemc =0; icemc < calib_clcemc_had.size(); icemc ++) {
        clcemc_hadsum += calib_clcemc_had.at(icemc).E();
        tmp_calib_clusterjet += calib_clcemc_had.at(icemc);
        tmp_calib_myjet += calib_clcemc_had.at(icemc);
      }
      for (int icemc =0; icemc < calib_clcemc_em.size(); icemc ++) {
        clcemc_emsum += calib_clcemc_em.at(icemc).E();
        tmp_calib_clusterjet += calib_clcemc_em.at(icemc);
        tmp_calib_myjet += calib_clcemc_em.at(icemc);
      }
...
*/      
      ...
      reco_clcemc_emEsum[reco_n]=clcemc_emsum;

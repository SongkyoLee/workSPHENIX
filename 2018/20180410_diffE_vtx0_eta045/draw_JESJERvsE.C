void draw_JESJERvsE(string inputType="final_qcd"){
  ...
  TLegend* leg1 = new TLegend(0.18, 0.20, 0.40, 0.35);
...
//  SetGraphStyle(gjesSS310[1],0,11);
  SetGraphStyle(gjesSS310[2],0,10);
...
//  SetGraphStyle(gjesAlframe[1],1,11);
  SetGraphStyle(gjesAlframe[2],1,10);
...
//  gjesAl[4]->Draw("P");
  ...
  infile_jerAlBefore.open("txt_jes/jer_sig_before_Al.out");
  ijer_Al=0;
  ...
  for (int im=0; im<nMeth; im++){
    infile_jerAl[im].open(Form("txt_jes/jer_sig_method0%d_Al.out",im+1));
    ijer_Al=0;
  ...
  SetGraphStyle(gjerSS310[2],0,10);
  ...
  SetGraphStyle(gjerAlframe[2],1,10);

void draw_JESJERvsE_RecoToTrue_merged(string ihcalType="Alframe", string note="R04"){
  ...
  infile_jesBefore.open(Form("txt_jet_RecoToTrue_merged/jes_before_%s_%s.out",ihcalType.c_str(),note.c_str()));
  ijes=0;
  ...
  for (int im=0; im<nMeth; im++){
    infile_jesAfter[im].open(Form("txt_jet_RecoToTrue_merged/jes_%s_%s_%s.out",szMeth[im].c_str(),ihcalType.c_str(),note.c_str()));
    ijes=0;
  ...
  c_JES->SaveAs(Form("outvsERecoToTrueMerged/JESvsE_%s_%s.pdf",ihcalType.c_str(),note.c_str()));
  ///////////////////////////////////////
  ...
  infile_jerBefore.open(Form("txt_jet_RecoToTrue_merged/jer_before_%s_%s.out",ihcalType.c_str(),note.c_str()));
  ijer=0;
  ...
  for (int im=0; im<nMeth; im++){
    infile_jerAfter[im].open(Form("txt_jet_RecoToTrue_merged/jer_%s_%s_%s.out",szMeth[im].c_str(),ihcalType.c_str(),note.c_str()));
    ijer=0;
  ...
  c_JER->SaveAs(Form("outvsERecoToTrueMerged/JERvsE_%s_%s.pdf",ihcalType.c_str(),note.c_str()));
  ///////////////////////////////////////
  
  TFile* fout =  new TFile(Form("outvsERecoToTrueMerged/JESvsE_%s_%s.root",ihcalType.c_str(),note.c_str()),"RECREATE");
  fout->cd();
  gjesBefore->Write();
  for (int im=0; im<nMeth; im++){   
    gjesAfter[im]->Write();

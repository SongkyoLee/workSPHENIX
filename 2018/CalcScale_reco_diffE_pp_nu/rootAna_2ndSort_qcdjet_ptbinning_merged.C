...
void rootAna_2ndSort_qcdjet_ptbinning_merged(string sampleType = "QCDJet",
                              ...
                              bool dodphicut=true)
{
  ...
  outTree->Branch("file_flag",&file_flag,"file_flag/I");
  ...
  for(int ii=0; ii<nEBin; ii++){
    ...
    file_weight =  totlumi/lumi[ii];
    cout << "file_weight = " << file_weight << endl;

    file_flag = ii+1; 
    string fname;
    fname = Form("./sortedRootFiles/sorted_%dto%d_%s_G4sPHENIX_jet4_%s_%s_%s_doetopcut%d_dodphicut%d.root",initfile,endfile,sampleType.c_str(),jetInE[ii].c_str(),ihcalType.c_str(),note.c_str(),(int)doetopcut,(int)dodphicut);

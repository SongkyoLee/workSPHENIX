void check_sortedRootFiles_pt_weight(){
  ...
  //// GENOnly noTreeCut + bad files removed (/pb) (265,266,278,267)
  //float lumi01 = 0.901274*(400*50)/(265*200);
  //float lumi02 = 19.0015*(400*50)/(266*200);  
  //float lumi03 = 339.592*(400*50)/(278*200);  
  //float lumi04 = 5903.88*(400*50)/(267*200);  
  
  //// GENOnly noTreeCut + bad files removed + truthjet4_pt>5 from merged.root (/pb)
  //float lumi01 = 0.0485595*0.901274*(400*50)/(265*200);
  //float lumi02 = 0.0548877*19.0015*(400*50)/(266*200);  
  //float lumi03 = 0.05882*339.592*(400*50)/(278*200);  
  //float lumi04 = 0.062519*5903.88*(400*50)/(267*200);  
  
  //GENOnly TreeCut pT > 5 GeV + bad files removed (/pb)
  float lumi01 = 0.928612*(400*50)/(274*200);
  float lumi02 = 19.566*(400*50)/(273*200);
  float lumi03 = 328.324*(400*50)/(270*200);
  float lumi04 = 6067.85*(400*50)/(276*200);
  ...
  t_true_01->Draw("true_pt>>h_true_pt_merged",Form("%f*(20<true_pt&&true_pt<30)",weight01));
  t_true_02->Draw("true_pt>>+h_true_pt_merged",Form("%f*(30<true_pt&&true_pt<40)",weight02));
  t_true_03->Draw("true_pt>>+h_true_pt_merged",Form("%f*(40<true_pt&&true_pt<50)",weight03));
  t_true_04->Draw("true_pt>>+h_true_pt_merged",Form("%f*(50<true_pt&&true_pt<60)",weight04));
  ...
  SetSoftHistStyle(h_true_pt_merged,0,0);
  h_true_pt_merged->Draw("p same");

  
  //TF1* tf_powerlaw = new TF1( "tf_powerlaw", "pow(10,[0]*x+[1])", 20,60);
  //TF1* tf_powerlaw = new TF1( "tf_powerlaw", "[0]*pow([1]*x,[2])", 20,60);
  //TF1* tf_powerlaw = new TF1( "tf_powerlaw", "[0]*pow([1]*x,[2])", 20,60);
  TF1* tf_powerlaw = new TF1( "tf_powerlaw", "[0]*pow([1]*x,[2])", 20,60);
  tf_powerlaw->SetParameter(0,1000);
  tf_powerlaw->SetParameter(1,10);
  tf_powerlaw->SetParameter(2,-1);
  h_true_pt_merged->Fit(tf_powerlaw,"","",20,60);
  tf_powerlaw->SetLineColor(kRed);
  tf_powerlaw->Draw("same");

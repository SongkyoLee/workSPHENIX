void comp_JESJERvsE_syst(string ihcalType="Al", string note="48fmR04"){
  ...
  SetSoftGraphStyle(gjesBefore,1,10);
  SetSoftGraphStyle(gjesAfter[0],1,0);
  ...
  //infile_jerAfter[1].open(Form("txt_jet_systup/jer_%s_%s_%s.out",szMeth[1].c_str(),ihcalType.c_str(),note.c_str()));
  //ijer=0;
  //if (infile_jerAfter[1].is_open()){
  //  while( getline (infile_jerAfter[1],szjer) ) { jerAfter[1][ijer] = atof(szjer.c_str()); ijer++; }
  //}
  //infile_jerAfter[1].open(Form("txt_jet_systdown/jer_%s_%s_%s.out",szMeth[1].c_str(),ihcalType.c_str(),note.c_str()));
  //ijer=0;
  //if (infile_jerAfter[1].is_open()){
  //  while( getline (infile_jerAfter[1],szjer) ) { jerAfter[1][ijer] = atof(szjer.c_str()); ijer++; }
  //}
  ...
  SetSoftGraphStyle(gjerBefore,1,10);
  SetSoftGraphStyle(gjerAfter[0],1,0);
  ...
  gjerSys->SetFillColorAlpha(kRed-10,0.5); 
  gjerSys->SetLineColor(kPink+3); 
  
  TCanvas* c_Sys = new TCanvas("c_Sys","",600,600);
  c_Sys->cd();
  SetSoftGraphStyle(gjerBefore,1,10);
  SetSoftGraphStyle(gjerAfter[0],1,0);
  SetSoftGraphStyle(gjerSys,1,0);
  gjerBefore->Draw("AP");
  gjerSys->Draw("5");
  gjerAfter[0]->Draw("P");

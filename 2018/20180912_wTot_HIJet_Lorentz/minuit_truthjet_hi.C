...
Double_t func(Double_t x1, Double_t x2, Double_t x3, Double_t *par);
...
void minuit_truthjet_hi(string sampleType = "HIJet",
                              	string ihcalType = "Al",
                              	string jetE = "20GeV",
                              	string note = "04fmR02",
                              	int initfile=0,
...
{  
  ...
  for (int ievt=0; ievt <nevt; ievt++){
    ...
    reco_jet_origE = reco_jet_cemcEsum + reco_jet_ihcalEsum + reco_jet_ohcalEsum;
    ...
    cemc_E.push_back(reco_jet_cemcEsum); 
  ...
  gMinuit->mnparm(0, "ihcal_scale", vstart[0], step[0], 1,10,ierflg);
  ...
  Double_t cemcScale = 1;
  Double_t cemcScaleErr = 0;
  Double_t ihcalScale;
  Double_t ihcalScaleErr;
  Double_t ohcalScale;
  Double_t ohcalScaleErr;
  //gMinuit->GetParameter(0,cemcScale,cemcScaleErr);
  gMinuit->GetParameter(0,ihcalScale,ihcalScaleErr);
  ...
  for (int ievt=0; ievt <nevt; ievt++){
    ...
    //reco_jet_caliE = reco_jet_clcemc_emEsum + reco_jet_clcemc_hadEsum + ihcalScale*reco_jet_ihcalEsum + ohcalScale*reco_jet_ohcalEsum;
    reco_jet_caliE = reco_jet_cemcEsum + ihcalScale*reco_jet_ihcalEsum + ohcalScale*reco_jet_ohcalEsum;
  ...
  c1->SaveAs(Form("minuit_truthjet_hi/%d_%d_%s_%s_%s_%s.pdf",initfile,endfile,sampleType.c_str(),jetE.c_str(),ihcalType.c_str(),note.c_str()));
  ...
  c2->SaveAs(Form("minuit_truthjet_hi/%d_%d_%s_%s_%s_%s_v2.pdf",initfile,endfile,sampleType.c_str(),jetE.c_str(),ihcalType.c_str(),note.c_str()));
...
Double_t func(Double_t x1, Double_t x2, Double_t x3, Double_t *par){
  ...
  //return x1 + x2 + par[0]*x3 + par[1]*x4; //SS310,Al
  //return x1 + par[0]*x2 + par[1]*x4; //Alframe
  //// cemc + B*ihcal + C*ohcal;
  return x1 + par[0]*x2 + par[1]*x3; //HI Jet
}
...
void fcn(Int_t &npar, Double_t *gin, Double_t &f, Double_t *param, Int_t iflag){
  ...
  for (ir=0;ir<true_E.size(); ir++){
    ...
    delta = (func(cemc_E[ir],ihcal_E[ir],ohcal_E[ir],param)-true_E[ir])/true_E[ir];
    chisq += delta*delta;
  } 
  f = chisq;
}

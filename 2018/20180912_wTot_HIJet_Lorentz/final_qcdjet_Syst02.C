...
void final_qcdjet_Syst02(string sampleType = "HIJet",
                              string ihcalType = "Al",
                              string jetE = "20GeV",
                              string note = "48fmR02",
                              int initfile=0,
                              ...
                              bool dodphicut=false)
{
  ...
  float vari=0.2;
  cemc_sf=cemc_sf*(1+vari);
  ihcal_sf=ihcal_sf*(1+vari);
  ohcal_sf=ohcal_sf*(1-vari);
...
//  out_tree->SetBranchAddress("reco_jet_clcemc_emEsum",&reco_jet_clcemc_emEsum);
  ...
  for (int ievt=0; ievt <nevt; ievt++){
    ...
    reco_jet_cemcEsum = reco_jet_cemcEsum*total_sf; 
    ...
    ihfrac = ( reco_jet_ihcalEsum + reco_jet_ohcalEsum ) / ( true_jet_e - (reco_jet_cemcEsum) );
    ...
    ehasymm = ( reco_jet_cemcEsum - (reco_jet_ihcalEsum + reco_jet_ohcalEsum) ) / ( reco_jet_cemcEsum + (reco_jet_ihcalEsum + reco_jet_ohcalEsum) );
    //ehfrac = ( reco_jet_clcemc_hadEsum + reco_jet_ihcalEsum + reco_jet_ohcalEsum ) / ( true_jet_e - reco_jet_clcemc_emEsum );
    ehfrac = ( reco_jet_cemcEsum + reco_jet_ihcalEsum + reco_jet_ohcalEsum ) / ( true_jet_e );
    ...
    totehasymm = ( reco_jet_cemcEsum - (reco_jet_ihcalEsum + reco_jet_ohcalEsum) ) / ( reco_jet_cemcEsum + (reco_jet_ihcalEsum + reco_jet_ohcalEsum) );
    //totehfrac = ( reco_jet_clcemc_emEsum + reco_jet_clcemc_hadEsum + reco_jet_ihcalEsum + reco_jet_ohcalEsum ) / ( true_jet_e );
    totehfrac = ( reco_jet_cemcEsum + reco_jet_ihcalEsum + reco_jet_ohcalEsum ) / ( true_jet_e );
    ...
    h_emfrac -> Fill(true_jet_emfrac,( reco_jet_cemcEsum + reco_jet_ihcalEsum + reco_jet_ohcalEsum )/true_jet_e);
    
    //h_JES->Fill(( reco_jet_clcemc_emEsum + reco_jet_clcemc_hadEsum + reco_jet_ihcalEsum + reco_jet_ohcalEsum )/true_jet_e);
    h_JES->Fill(( reco_jet_cemcEsum + reco_jet_ihcalEsum + reco_jet_ohcalEsum )/true_jet_e);
...
//  palette->SetX2NDC(1-margin*3);
  ...
  ////////////////////////////////////////
  if (total_sf !=1) {
    c1->SaveAs(Form("outFinal%sSyst02/final2D_%s_%s_%s_%s_%.0f_%.0f_%.0f_wTot%.0f.pdf",ihcalType.c_str(),sampleType.c_str(),jetE.c_str(),ihcalType.c_str(),note.c_str(),100*cemc_sf,100*ihcal_sf,100*ohcal_sf,100*total_sf));
  } else {
    c1->SaveAs(Form("outFinal%sSyst02/final2D_%s_%s_%s_%s_%.0f_%.0f_%.0f.pdf",ihcalType.c_str(),sampleType.c_str(),jetE.c_str(),ihcalType.c_str(),note.c_str(),100*cemc_sf,100*ihcal_sf,100*ohcal_sf));
  }  
  ...
  if (total_sf !=1) {
    c2->SaveAs(Form("outFinal%sSyst02/final1D_%s_%s_%s_%s_%.0f_%.0f_%.0f_wTot%.0f.pdf",ihcalType.c_str(),sampleType.c_str(),jetE.c_str(),ihcalType.c_str(),note.c_str(),100*cemc_sf,100*ihcal_sf,100*ohcal_sf,100*total_sf));
  } else {
    c2->SaveAs(Form("outFinal%sSyst02/final1D_%s_%s_%s_%s_%.0f_%.0f_%.0f.pdf",ihcalType.c_str(),sampleType.c_str(),jetE.c_str(),ihcalType.c_str(),note.c_str(),100*cemc_sf,100*ihcal_sf,100*ohcal_sf));
  }
  ...
  //cout << "jer_sig = " << sig_jer << endl;
	...
	TFile* fout;
  //fout = new TFile(Form("outFinal%sSyst02/final_%s_%s_%s_%s_%.0f_%.0f_%.0f.root",ihcalType.c_str(),sampleType.c_str(),jetE.c_str(),ihcalType.c_str(),note.c_str(),100*cemc_sf,100*ihcal_sf,100*ohcal_sf),"RECREATE");
  if (cemc_sf==1 && ihcal_sf==1 && ohcal_sf==1) { 
    fout = new TFile(Form("outFinal%sSyst02/final_%s_%s_%s_%s_before.root",ihcalType.c_str(),sampleType.c_str(),jetE.c_str(),ihcalType.c_str(),note.c_str()),"RECREATE"); 
  } else { 
    if (total_sf!=1){
      fout = new TFile(Form("outFinal%sSyst02/final_%s_%s_%s_%s_truthjet_wTot.root",ihcalType.c_str(),sampleType.c_str(),jetE.c_str(),ihcalType.c_str(),note.c_str()),"RECREATE"); //tmp 
    } else {
      fout = new TFile(Form("outFinal%sSyst02/final_%s_%s_%s_%s_truthjet.root",ihcalType.c_str(),sampleType.c_str(),jetE.c_str(),ihcalType.c_str(),note.c_str()),"RECREATE"); //tmp
    }   
  }

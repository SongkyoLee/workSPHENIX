void draw_JESJER_kpp(string res="R04"){
  ...
  else if (res.compare("R02")==0) { jerymin =0.0; jerymax = 0.5; }
  else {cout << "Warning:: select  R04 or R02" << endl; return; }
  ...
  infile_jesSS310Before.open(Form("txt_jes/jes_mean_QCDJet_%s.out",res.c_str()));
  ijes_SS310=0;
  ...
  infile_jesAlframeBefore.open(Form("txt_jes/jes_mean_ObjectiveKPP_%s.out",res.c_str()));
  ijes_Alframe=0;
  ...
  infile_jesAlBefore.open(Form("txt_jes/jes_mean_ThresholdKPP_%s.out",res.c_str()));
  ijes_Al=0;
  ...
  leg2->AddEntry(gjesSS310Before,"Default","lp");
  leg2->AddEntry(gjesAlframeBefore,"Objective KPP","lp");
  leg2->AddEntry(gjesAlBefore,"Threshold KPP","lp");
...
//  dashedLine(0.0,1.0,70.0,1.0,1,1);
  ...
  c_JES->SaveAs(Form("out_figs/JESvsE_kpp_%s.pdf",res.c_str()));
  ...
  infile_jerSS310Before.open(Form("txt_jes/jes_sigmean_QCDJet_%s.out",res.c_str()));
  ijer_SS310=0;
  ...
  infile_jerAlframeBefore.open(Form("txt_jes/jes_sigmean_ObjectiveKPP_%s.out",res.c_str()));
  ijer_Alframe=0;
  ...
  infile_jerAlBefore.open(Form("txt_jes/jes_sigmean_ThresholdKPP_%s.out",res.c_str()));
  ijer_Al=0;
...
//  dashedLine(0.0,1.0,70.0,1.0,1,1);
  ...
  c_JER->SaveAs(Form("out_figs/JERvsE_kpp_%s.pdf",res.c_str()));
/*

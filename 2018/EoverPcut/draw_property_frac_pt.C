{
 ...
 TH1D* h_pt_k0 = (TH1D*)f1->Get("h_pt_k0");
  ...
  h_pt_epm->SetFillColor(kAzure+1);
  h_pt_epm->SetFillStyle(3445);
  h_pt_epm->Draw("same");  
  ...
  leg->AddEntry(h_pt_epm,"e^{#pm}, #gamma","l");
  ...
  leg2->AddEntry(h_pt_epm,"e^{#pm}, #gamma","l");
  leg2->AddEntry(h_pt_neutron,"neutral hadron","l");
  leg2->Draw();
  
  c2->SaveAs("out1DHist/property_fract_pt_log.pdf");

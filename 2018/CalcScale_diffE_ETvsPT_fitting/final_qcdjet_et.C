...
void final_qcdjet_et(string sampleType = "QCDJet",
                              string ihcalType = "Al",
                              string jetE = "20GeV",
                              int initfile=0,
                              int endfile =1000,
                              ...
                              bool dodphicut=true)
{
  ...
  TH1D* h_JES = new TH1D("h_JES",";Jet E_{T, Reco} / Jet E_{T, Truth};Counts",100,0,2);
  TH1D* h_JER = new TH1D("h_JER",";(Jet E_{T, Reco}-Jet E_{T, Truth}) / Jet E_{T, Reco};Counts",100,-1,1);
  ...
  out_tree->SetBranchAddress("true_jet_eta",&true_jet_eta);
  out_tree->SetBranchAddress("true_jet_phi",&true_jet_phi);
  ...
  out_tree->SetBranchAddress("reco_jet_eta",&reco_jet_eta);
  out_tree->SetBranchAddress("reco_jet_phi",&reco_jet_phi);
  ...
  for (int ievt=0; ievt <nevt; ievt++){
    ...
    //h_emfrac -> Fill(true_jet_emfrac,totehfrac);
    ...
    //h_JER->Fill(( reco_jet_clcemc_emEsum + reco_jet_clcemc_hadEsum + reco_jet_ihcalEsum + reco_jet_ohcalEsum - true_jet_e)/ ( reco_jet_clcemc_emEsum + reco_jet_clcemc_hadEsum + reco_jet_ihcalEsum + reco_jet_ohcalEsum ));
		h_JES->Fill(( (reco_jet_clcemc_emEsum + reco_jet_clcemc_hadEsum + reco_jet_ihcalEsum + reco_jet_ohcalEsum)/cosh(reco_jet_eta) )/(true_jet_e/cosh(true_jet_eta)) );
		h_JER->Fill(( ((reco_jet_clcemc_emEsum + reco_jet_clcemc_hadEsum + reco_jet_ihcalEsum + reco_jet_ohcalEsum)/cosh(reco_jet_eta)) - (true_jet_e/cosh(true_jet_eta))) / ( (reco_jet_clcemc_emEsum + reco_jet_clcemc_hadEsum + reco_jet_ihcalEsum + reco_jet_ohcalEsum )/cosh(reco_jet_eta)) );
  }
  ...
  prof_IHAsymm->SetMarkerSize(1.2);
...
//  palette->SetX2NDC(1-margin*3);
  ...
  prof_EHAsymm->SetMarkerSize(1.2);
  ...
  prof_totEHAsymm->SetMarkerSize(1.2);
  ...
  ////////////////////////////////////////
  if (total_sf !=1) {
    c1->SaveAs(Form("outFinal%s/final2D_wTot%.0f_%s_%s_%s_%.0f_%.0f_%.0f.pdf",ihcalType.c_str(),100*total_sf,sampleType.c_str(),jetE.c_str(),ihcalType.c_str(),100*cemc_sf,100*ihcal_sf,100*ohcal_sf));
  } else {
    c1->SaveAs(Form("outFinal%s/final2D_%s_%s_%s_%.0f_%.0f_%.0f.pdf",ihcalType.c_str(),sampleType.c_str(),jetE.c_str(),ihcalType.c_str(),100*cemc_sf,100*ihcal_sf,100*ohcal_sf));
  }

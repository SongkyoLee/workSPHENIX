void draw_JESJERvsE_withEM(string ihcalType="SS310", string note="R04"){
  ...
  infile_jesBefore.open(Form("txt_jet_withEM/jes_before_%s_%s.out",ihcalType.c_str(),note.c_str()));
  ijes=0;
  ...
  for (int im=0; im<nMeth; im++){
    infile_jesAfter[im].open(Form("txt_jet_withEM/jes_%s_%s_%s.out",szMeth[im].c_str(),ihcalType.c_str(),note.c_str()));
    ijes=0;
  ...
  SetSoftGraphStyle(gjesAfter[2],2,12);
  SetSoftGraphStyle(gjesAfter[3],2,13);
  ...
  c_JES->SaveAs(Form("outvsE_withEM/JESvsE_%s_%s.pdf",ihcalType.c_str(),note.c_str()));
  ///////////////////////////////////////
  ...
  infile_jerBefore.open(Form("txt_jet_withEM/jer_before_%s_%s.out",ihcalType.c_str(),note.c_str()));
  ijer=0;
  ...
  for (int im=0; im<nMeth; im++){
    infile_jerAfter[im].open(Form("txt_jet_withEM/jer_%s_%s_%s.out",szMeth[im].c_str(),ihcalType.c_str(),note.c_str()));
    ijer=0;
  ...
  SetSoftGraphStyle(gjerAfter[2],2,12);
  SetSoftGraphStyle(gjerAfter[3],2,13);
  ...
  c_JER->SaveAs(Form("outvsE_withEM/JERvsE_%s_%s.pdf",ihcalType.c_str(),note.c_str()));
  ///////////////////////////////////////
  
  TFile* fout =  new TFile(Form("outvsE_withEM/JESvsE_%s_%s.root",ihcalType.c_str(),note.c_str()),"RECREATE");
  fout->cd();
  gjesBefore->Write();
  for (int im=0; im<nMeth; im++){   
    gjesAfter[im]->Write();

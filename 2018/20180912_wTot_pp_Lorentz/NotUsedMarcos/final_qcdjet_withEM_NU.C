...
void final_qcdjet_withEM(string sampleType = "QCDJet",
...
//  }
  ...
  for (int ievt=0; ievt <nevt; ievt++){
    ...
    //// scaling
    reco_jet_clcemc_emEsum = reco_jet_clcemc_emEsum*cemcem_sf;
...
//  palette->SetX2NDC(1-margin*3);
  ...
  ////////////////////////////////////////
  if (total_sf !=1) {
    c1->SaveAs(Form("outFinal%s_withEM/final2D_wTot%.0f_%s_%s_%s_%s_%.0f_%.0f_%.0f_%.0f.pdf",ihcalType.c_str(),100*total_sf,sampleType.c_str(),jetE.c_str(),ihcalType.c_str(),note.c_str(),100*cemc_sf,100*ihcal_sf,100*ohcal_sf,100*cemcem_sf));
  } else {
    c1->SaveAs(Form("outFinal%s_withEM/final2D_%s_%s_%s_%s_%.0f_%.0f_%.0f_%.0f.pdf",ihcalType.c_str(),sampleType.c_str(),jetE.c_str(),ihcalType.c_str(),note.c_str(),100*cemc_sf,100*ihcal_sf,100*ohcal_sf,100*cemcem_sf));
  }
...
*/
  ...
  if (total_sf !=1) {
    c2->SaveAs(Form("outFinal%s_withEM/final1D_wTot%.0f_%s_%s_%s_%s_%.0f_%.0f_%.0f_%.0f.pdf",ihcalType.c_str(),100*total_sf,sampleType.c_str(),jetE.c_str(),ihcalType.c_str(),note.c_str(),100*cemc_sf,100*ihcal_sf,100*ohcal_sf,100*cemcem_sf));
  } else {
    c2->SaveAs(Form("outFinal%s_withEM/final1D_%s_%s_%s_%s_%.0f_%.0f_%.0f_%.0f.pdf",ihcalType.c_str(),sampleType.c_str(),jetE.c_str(),ihcalType.c_str(),note.c_str(),100*cemc_sf,100*ihcal_sf,100*ohcal_sf,100*cemcem_sf));
  }
  ...
  cout << "cemcEmScale = " << cemcem_sf << endl;

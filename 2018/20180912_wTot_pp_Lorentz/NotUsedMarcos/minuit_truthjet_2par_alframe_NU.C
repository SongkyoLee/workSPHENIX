...
void minuit_truthjet_2par_alframe(string sampleType = "QCDJet",
...
{  
  ...
  static Double_t vstart[2] = {1.0}; //staring values
  static Double_t step[2] = {0.01}; //step
  //gMinuit->mnparm(0, "cemc_scale", vstart[0], step[0], 1,10,ierflg);
  gMinuit->mnparm(0, "ohcal_scale", vstart[0], step[0], 1,10,ierflg);
  ...
  gMinuit->GetParameter(0,ohcalScale,ohcalScaleErr);
  ...
  for (int ievt=0; ievt <nevt; ievt++){
    ...
    reco_jet_caliE = reco_jet_clcemc_emEsum + reco_jet_clcemc_hadEsum + ohcalScale*reco_jet_ohcalEsum;
  ...
  c1->SaveAs(Form("minuit_truthjet_2par/%d_%d_%s_%s_%s_%s.pdf",initfile,endfile,sampleType.c_str(),jetE.c_str(),ihcalType.c_str(),note.c_str()));
  ...
  c2->SaveAs(Form("minuit_truthjet_2par/%d_%d_%s_%s_%s_%s_v2.pdf",initfile,endfile,sampleType.c_str(),jetE.c_str(),ihcalType.c_str(),note.c_str()));

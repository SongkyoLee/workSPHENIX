...
void minuit_truthjet_4par(string sampleType = "QCDJet",
...
{  
  ...
  gMinuit->mnparm(3, "cemcem_scale", vstart[3], step[3], 1,10,ierflg);
  ...
  Double_t cemcEmScale;
  Double_t cemcEmScaleErr;
  ...
  gMinuit->GetParameter(3,cemcEmScale,cemcEmScaleErr);
  ...
  for (int ievt=0; ievt <nevt; ievt++){
    ...
    reco_jet_caliE = cemcEmScale*reco_jet_clcemc_emEsum + cemcScale*reco_jet_clcemc_hadEsum + ihcalScale*reco_jet_ihcalEsum + ohcalScale*reco_jet_ohcalEsum;
  ...
  c1->SaveAs(Form("minuit_truthjet_4par/%d_%d_%s_%s_%s_%s.pdf",initfile,endfile,sampleType.c_str(),jetE.c_str(),ihcalType.c_str(),note.c_str()));
  ...
  c2->SaveAs(Form("minuit_truthjet_4par/%d_%d_%s_%s_%s_%s_v2.pdf",initfile,endfile,sampleType.c_str(),jetE.c_str(),ihcalType.c_str(),note.c_str()));

  cout << "cemcScale = " << cemcScale << endl;
  cout << "ihcalScale = " << ihcalScale << endl;
  cout << "ohcalScale = " << ohcalScale << endl;
  cout << "cemcEmScale = " << cemcEmScale << endl;
...
Double_t func(Double_t x1, Double_t x2, Double_t x3, Double_t x4, Double_t *par){
  ...
  return par[3]*x1 + par[0]*x2 + par[1]*x3 + par[2]*x4; //SS310

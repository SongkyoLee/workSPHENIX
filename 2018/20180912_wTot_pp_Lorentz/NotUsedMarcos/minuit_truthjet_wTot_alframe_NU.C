...
void minuit_truthjet_wTot_alframe(string sampleType = "QCDJet",
...
{  
  ...
  gMinuit->mnparm(2, "total_scale", vstart[1], step[1], 1,10,ierflg);
  ...
  gMinuit->GetParameter(2,totalScale,totalScaleErr);
  ...
  for (int ievt=0; ievt <nevt; ievt++){
    ...
    reco_jet_caliE = totalScale*(reco_jet_clcemc_emEsum + cemcScale*reco_jet_clcemc_hadEsum + ohcalScale*reco_jet_ohcalEsum);
...
Double_t func(Double_t x1, Double_t x2, Double_t x3, Double_t x4, Double_t *par){
  ...
  return par[2]*(x1 + par[0]*x2 + par[1]*x4); //Alframe
}

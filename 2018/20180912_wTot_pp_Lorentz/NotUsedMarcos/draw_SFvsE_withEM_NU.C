void draw_SFvsE_withEM(string ihcalType="SS310", string note="R04"){
  ...
  szMeth[2]= "truthjet_4par";
  szMeth[3]= "truthjet_wTot";
  ...
  for (int im=0; im<nMeth; im++){
    infile_cemc[im].open(Form("txt_scale_withEM/cemcScale_%s_%s_%s.out",szMeth[im].c_str(),ihcalType.c_str(),note.c_str()));
    icemc=0;
  ...
  for (int im=0; im<nMeth; im++){
    infile_ihcal[im].open(Form("txt_scale_withEM/ihcalScale_%s_%s_%s.out",szMeth[im].c_str(),ihcalType.c_str(),note.c_str()));
    iihcal=0;
  ...
  for (int im=0; im<nMeth; im++){
    infile_ohcal[im].open(Form("txt_scale_withEM/ohcalScale_%s_%s_%s.out",szMeth[im].c_str(),ihcalType.c_str(),note.c_str()));
    iohcal=0;
  ...
  string szcemcem;
  int icemcem=0;
  ifstream infile_cemcem[nMeth];
  for (int im=0; im<nMeth; im++){
    infile_cemcem[im].open(Form("txt_scale_withEM/cemcEmScale_%s_%s_%s.out",szMeth[im].c_str(),ihcalType.c_str(),note.c_str()));
    icemcem=0;
    if (infile_cemcem[im].is_open()){
      while( getline (infile_cemcem[im],szcemcem) ) { cemcem[im][icemcem] = atof(szcemcem.c_str()); icemcem++; }
    }
  }
  ...
  for (int im=0; im<nMeth; im++){   
    for (int ie=0; ie<nEBin; ie++){
      ...
      cout << "cemcem["<<im<<"]["<<ie<<"] = " << cemcem[im][ie] << endl;
    }
  }
  ...
  TGraphAsymmErrors* gcemcem[nMeth];
  for (int im=0; im<nMeth; im++){   
  ...
  leg1->AddEntry(gcemcem[0],"EMCAL^{EM}","lp");
  leg1->Draw();
  ...
  c_sf->SaveAs(Form("outvsE_withEM/SFvsE_%s_%s.pdf",ihcalType.c_str(),note.c_str()));

  ///////////////////////////////////////
  
  TFile* fout =  new TFile(Form("outvsE_withEM/SFvsE_%s_%s.root",ihcalType.c_str(),note.c_str()),"RECREATE");
  fout->cd();
  for (int im=0; im<nMeth; im++){   
    gcemc[im]->Write();
    gihcal[im]->Write();

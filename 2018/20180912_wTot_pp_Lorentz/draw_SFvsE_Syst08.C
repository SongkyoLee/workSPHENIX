void draw_SFvsE_Syst08(string ihcalType="SS310", string note="R04"){
  ...
  for (int im=0; im<nMeth; im++){
    infile_cemc[im].open(Form("txt_scale_syst08/cemcScale_%s_%s_%s.out",szMeth[im].c_str(),ihcalType.c_str(),note.c_str()));
    icemc=0;
  ...
  for (int im=0; im<nMeth; im++){
    infile_ihcal[im].open(Form("txt_scale_syst08/ihcalScale_%s_%s_%s.out",szMeth[im].c_str(),ihcalType.c_str(),note.c_str()));
    iihcal=0;
  ...
  for (int im=0; im<nMeth; im++){
    infile_ohcal[im].open(Form("txt_scale_syst08/ohcalScale_%s_%s_%s.out",szMeth[im].c_str(),ihcalType.c_str(),note.c_str()));
    iohcal=0;
  ...
  c_sf->SaveAs(Form("outvsE_Syst08/SFvsE_%s_%s.pdf",ihcalType.c_str(),note.c_str()));

  ///////////////////////////////////////
  
  TFile* fout =  new TFile(Form("outvsE_Syst08/SFvsE_%s_%s.root",ihcalType.c_str(),note.c_str()),"RECREATE");
  fout->cd();
  for (int im=0; im<nMeth; im++){   
    gcemc[im]->Write();
    gihcal[im]->Write();

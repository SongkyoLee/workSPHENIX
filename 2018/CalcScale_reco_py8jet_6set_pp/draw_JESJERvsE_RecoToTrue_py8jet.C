void draw_JESJERvsE_RecoToTrue_py8jet(string ihcalType="Alframe", string note="R04"){
  ...
  infile_jesBefore.open(Form("txt_jet_RecoToTrue_py8jet/jes_before_%s_%s.out",ihcalType.c_str(),note.c_str()));
  ijes=0;
  ...
  for (int im=0; im<nMeth; im++){
    infile_jesAfter[im].open(Form("txt_jet_RecoToTrue_py8jet/jes_%s_%s_%s.out",szMeth[im].c_str(),ihcalType.c_str(),note.c_str()));
    ijes=0;
  ...
  double xPntF[nEBin] = {25, 35, 45};
  double exF[nEBin] = {5, 5, 5};
  double eyF[nEBin] = {0, 0, 0};
  double jesBeforeF[nEBin];
  double jesAfterF[nMeth][nEBin];
  
  for (int ie=0; ie<nEBinF; ie++){
    jesBeforeF[ie] = jesBefore[ie];
    for (int im=0; im<nMeth; im++){   
      jesAfterF[im][ie] = jesAfter[im][ie];
    }
  }
  
  //TGraphAsymmErrors* gjesBefore = new TGraphAsymmErrors(nEBin, xPnt, jesBefore, ex, ex, ey, ey);
  TGraphAsymmErrors* gjesBefore = new TGraphAsymmErrors(nEBinF, xPntF, jesBeforeF, exF, exF, eyF, eyF);
  ...
  for (int im=0; im<nMeth; im++){
    ...
    gjesAfter[im] = new TGraphAsymmErrors(nEBinF, xPntF, jesAfterF[im], exF, exF, eyF, eyF);
  ...
  c_JES->SaveAs(Form("outvsERecoToTruePy8jet/JESvsE_%s_%s.pdf",ihcalType.c_str(),note.c_str()));
  ///////////////////////////////////////
  ...
  infile_jerBefore.open(Form("txt_jet_RecoToTrue_py8jet/jer_before_%s_%s.out",ihcalType.c_str(),note.c_str()));
  ijer=0;
  ...
  for (int im=0; im<nMeth; im++){
    infile_jerAfter[im].open(Form("txt_jet_RecoToTrue_py8jet/jer_%s_%s_%s.out",szMeth[im].c_str(),ihcalType.c_str(),note.c_str()));
    ijer=0;
  ...
  ///// Changed Binning
  double jerBeforeF[nEBin];
  double jerAfterF[nMeth][nEBin];
  
  for (int ie=0; ie<nEBinF; ie++){
    jerBeforeF[ie] = jerBefore[ie];
    for (int im=0; im<nMeth; im++){   
      jerAfterF[im][ie] = jerAfter[im][ie];
    }
  }
  
  //TGraphAsymmErrors* gjerBefore = new TGraphAsymmErrors(nEBin, xPnt, jerBefore, ex, ex, ey, ey);
  TGraphAsymmErrors* gjerBefore = new TGraphAsymmErrors(nEBinF, xPntF, jerBeforeF, exF, exF, eyF, eyF);
  ...
  for (int im=0; im<nMeth; im++){
    ...
    gjerAfter[im] = new TGraphAsymmErrors(nEBinF, xPntF, jerAfterF[im], exF, exF, eyF, eyF);
  ...
  c_JER->SaveAs(Form("outvsERecoToTruePy8jet/JERvsE_%s_%s.pdf",ihcalType.c_str(),note.c_str()));
  ///////////////////////////////////////
  
  TFile* fout =  new TFile(Form("outvsERecoToTruePy8jet/JESvsE_%s_%s.root",ihcalType.c_str(),note.c_str()),"RECREATE");
  fout->cd();
  gjesBefore->Write();
  for (int im=0; im<nMeth; im++){   
    gjesAfter[im]->Write();

...
void rootAna_2ndSort_qcdjet_lorentz_ptbinning_py8jet(string sampleType = "py8Jet",
                              string ihcalType = "Al",
                              string jetInE = "allGeV",
                              string jetOutE = "noGeV",
                              string note = "R04",
                              int initfile=0,
                              int endfile =12000,
                              bool doetopcut=true,
                              ...
                              bool doscale=true)
{
  ...
  //string foutname = Form("./2ndSortedRootFilesPtBinningMerged/2ndSorted_%dto%d_%s_G4sPHENIX_jet4_%s_%s_%s_doetopcut%d_dodphicut%d.root",initfile,endfile,sampleType.c_str(),jetOutE.c_str(),ihcalType.c_str(),note.c_str(),(int)doetopcut,(int)dodphicut);
  //string foutname = Form("./2ndSortedRootFilesPtBinningPy8jet/2ndSorted_%dto%d_%s_G4sPHENIX_jet4_%s_%s_%s_doetopcut%d_dodphicut%d.root",initfile,endfile,sampleType.c_str(),jetOutE.c_str(),ihcalType.c_str(),note.c_str(),(int)doetopcut,(int)dodphicut);
  string foutname = Form("./2ndSortedRootFilesLorentzPtBinningPy8jet/2ndSorted_%dto%d_%s_G4sPHENIX_jet4_%s_%s_%s_doetopcut%d_dodphicut%d_doscale%d.root",initfile,endfile,sampleType.c_str(),jetOutE.c_str(),ihcalType.c_str(),note.c_str(),(int)doetopcut,(int)dodphicut,(int)doscale);
  TFile* fout = new TFile( foutname.c_str(), "RECREATE");
  TTree* outTree = new TTree("out_tree","out_tree");
  
  int evt=1;
  float file_weight = -999; 
  float true_jet_e, true_jet_p, true_jet_pt, true_jet_eta, true_jet_phi, true_jet_m;
  float true_jet_lcons_z, true_jet_emfrac;
  int true_jet_cons_n, true_jet_lcons_pid;

  float reco_jet_e, reco_jet_p, reco_jet_pt, reco_jet_eta, reco_jet_phi, reco_jet_m;
  ...
  float orig_reco_jet_e, orig_reco_jet_p, orig_reco_jet_pt, orig_reco_jet_eta, orig_reco_jet_phi, orig_reco_jet_m;

  outTree->Branch("evt",&evt,"evt/I");
  outTree->Branch("file_weight",&file_weight,"file_weight/F");
  ...
  outTree->Branch("orig_reco_jet_e",&orig_reco_jet_e,"orig_reco_jet_e/F");
  outTree->Branch("orig_reco_jet_p",&orig_reco_jet_p,"orig_reco_jet_p/F");
  outTree->Branch("orig_reco_jet_pt",&orig_reco_jet_pt,"orig_reco_jet_pt/F");
  outTree->Branch("orig_reco_jet_eta",&orig_reco_jet_eta,"orig_reco_jet_eta/F");
  outTree->Branch("orig_reco_jet_phi",&orig_reco_jet_phi,"orig_reco_jet_phi/F");
  outTree->Branch("orig_reco_jet_m",&orig_reco_jet_m,"orig_reco_jet_m/F");

  ////////////////////////////////////// 
  //// read-in files
  ////////////////////////////////////// 
//  const int nEBin = 4;
//  string jetInE[nEBin] = {"20GeV", "30GeV", "40GeV", "50GeV"};
//  float totlumi = 1;
//  //float totlumi = 45; //45 /pb
//  float lumi[nEBin];
//  //GENOnly TreeCut pT > 5 GeV + bad files removed (/pb)
//  lumi[0] = 0.928612*(400*50)/(274*200);
//  lumi[1] = 19.566*(400*50)/(273*200);
//  lumi[2] = 328.324*(400*50)/(270*200);
//  lumi[3] = 6067.85*(400*50)/(276*200);

    file_weight =  1.;
    cout << "file_weight = " << file_weight << endl;
    ...
    //fname = Form("./sortedRootFiles/sorted_%dto%d_%s_G4sPHENIX_jet4_%s_%s_%s_doetopcut%d_dodphicut%d.root",initfile,endfile,sampleType.c_str(),jetInE.c_str(),ihcalType.c_str(),note.c_str(),(int)doetopcut,(int)dodphicut);
    fname = Form("./sortedRootFilesLorentz/sorted_%dto%d_%s_G4sPHENIX_jet4_%s_%s_%s_doetopcut%d_dodphicut%d_doscale%d.root",initfile,endfile,sampleType.c_str(),jetInE.c_str(),ihcalType.c_str(),note.c_str(),(int)doetopcut,(int)dodphicut,(int)doscale);
    ...
    //true_tree->SetWeight(file_weight);

    if (true_tree->GetEntries() != reco_tree->GetEntries()) {
      cout <<"WARNING!! Entries of true_tree and reco_tree do not match!"<<endl; 
      cout << "true_tree->GetEntries() = " << true_tree->GetEntries() << endl; 
      cout << "reco_tree->GetEntries() = " << reco_tree->GetEntries() << endl; 
      return;
    }
  ...
  true_tree->SetBranchAddress("true_m",&true_m);
  ...
  true_tree->SetBranchAddress("reco_m",&reco_m);
  ...
  true_tree->SetBranchAddress("orig_reco_e",&orig_reco_e); 
  true_tree->SetBranchAddress("orig_reco_p",&orig_reco_p); 
  true_tree->SetBranchAddress("orig_reco_pt",&orig_reco_pt); 
  true_tree->SetBranchAddress("orig_reco_eta",&orig_reco_eta); 
  true_tree->SetBranchAddress("orig_reco_phi",&orig_reco_phi); 
  true_tree->SetBranchAddress("orig_reco_m",&orig_reco_m); 
  ...
  //for (int ievt=0; ievt<50; ievt++){ 
    ...
    for (int itrue=0; itrue<true_n; itrue++){
      ...
      //if (jetOutE.compare("noGeV")==0){ if (!(true_pt[itrue] > 0)) continue;}
      //else if (jetOutE.compare("allGeV")==0){ if (!(true_pt[itrue] > 20)) continue;}
      //else if (jetOutE.compare("20GeV")==0){ if (!(true_pt[itrue] > 20 && true_pt[itrue] <30)) continue;}
      //else if (jetOutE.compare("30GeV")==0){ if (!(true_pt[itrue] > 30 && true_pt[itrue] <40)) continue;}
      //else if (jetOutE.compare("40GeV")==0){ if (!(true_pt[itrue] > 40 && true_pt[itrue] <50)) continue;}
      //else if (jetOutE.compare("50GeV")==0){ if (!(true_pt[itrue] > 50 && true_pt[itrue] <60)) continue;}
      //else { cout << "KYO: select among 20-50GeV" << endl; return;}
      //if (file_flag ==1) { if (!(true_pt[itrue] > 20 && true_pt[itrue] <30)) continue; }
      //else if (file_flag ==2) { if (!(true_pt[itrue] > 30 && true_pt[itrue] <40)) continue; }
      //else if (file_flag ==3) { if (!(true_pt[itrue] > 40 && true_pt[itrue] <50)) continue; }
      //else if (file_flag ==4) { if (!(true_pt[itrue] > 50 && true_pt[itrue] <60)) continue; }
      //else { cout << "throw away truth jet " <<endl; }
      ...
      for (int ireco=0; ireco<reco_n; ireco++){
        ...
        if (tmp_jet_dR < match_jet_dR){
          ...
          orig_reco_jet_e = orig_reco_e[ireco];
          orig_reco_jet_p = orig_reco_p[ireco];
          orig_reco_jet_pt = orig_reco_pt[ireco];
          orig_reco_jet_eta = orig_reco_eta[ireco];
          orig_reco_jet_phi = orig_reco_phi[ireco];
          orig_reco_jet_m = orig_reco_m[ireco];
        }
      } //EOL over reco_n 

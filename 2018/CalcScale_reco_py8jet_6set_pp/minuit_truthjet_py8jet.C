...
void minuit_truthjet_py8jet(string sampleType = "py8Jet",
                              	string ihcalType = "Al",
                              	string jetE = "40GeV",
                              	string note = "R04",
                              	int initfile=0,
...
{  
  ...
  //string fname = Form("./2ndSortedRootFilesPtBinning/2ndSorted_%dto%d_%s_G4sPHENIX_jet4_%s_%s_%s_doetopcut%d_dodphicut%d.root",initfile,endfile,sampleType.c_str(),jetE.c_str(),ihcalType.c_str(),note.c_str(),(int)doetopcut,(int)dodphicut);
  //string fname = Form("./2ndSortedRootFilesPtBinningMerged/2ndSorted_%dto%d_%s_G4sPHENIX_jet4_noGeV_%s_%s_doetopcut%d_dodphicut%d.root",initfile,endfile,sampleType.c_str(),ihcalType.c_str(),note.c_str(),(int)doetopcut,(int)dodphicut);
  string fname = Form("./2ndSortedRootFilesPtBinningPy8jet/2ndSorted_%dto%d_%s_G4sPHENIX_jet4_noGeV_%s_%s_doetopcut%d_dodphicut%d.root",initfile,endfile,sampleType.c_str(),ihcalType.c_str(),note.c_str(),(int)doetopcut,(int)dodphicut);
  TFile* fin = new TFile( fname.c_str(), "READ");
  if(!fin->IsOpen()) { cout <<"WARNING!! file not found"<<endl; return;};
  TTree* out_tree = (TTree*)fin->Get("out_tree");

  //// before 
  TH1D* h_origRESTruthJet = new TH1D("h_origRESTruthJet",";(E_{Jet}^{Reco} - E_{Jet}^{Truth}) / E_{Jet}^{Truth};Counts",100,-1.5,1.5);
  TH2D* h_emfrac_origRESTruthJet = new TH2D("h_emfrac_origRESTruthJet",";Truth Jet EM fraction;(E_{Jet}^{Reco} - E_{Jet}^{Truth}) / E_{Jet}^{Truth}",101,-0.05,1.05,100,-1.5,1.5);
  TH2D* h_emfrac_origJESTruthJet = new TH2D("h_emfrac_origJESTruthJet",";Truth Jet EM fraction;E_{Jet}^{Reco} / E_{Jet}^{Truth}",101,-0.05,1.05,100,0,2);
  //// after
  TH1D* h_RESTruthJet = new TH1D("h_RESTruthJet",";(E_{Jet}^{Reco} - E_{Jet}^{Truth}) / E_{Jet}^{Truth};Counts",100,-1.5,1.5);
  TH2D* h_emfrac_RESTruthJet = new TH2D("h_emfrac_RESTruthJet",";Truth Jet EM fraction;(E_{Jet}^{Reco} - E_{Jet}^{Truth}) / E_{Jet}^{Truth}",101,-0.05,1.05,100,-1.5,1.5);
  TH2D* h_emfrac_JESTruthJet = new TH2D("h_emfrac_JESTruthJet",";Truth Jet EM fraction;E_{Jet}^{Reco} / E_{Jet}^{Truth}",101,-0.05,1.05,100,0,2);
  ...
  for (int ievt=0; ievt <nevt; ievt++){
    ...
    h_origRESTruthJet->Fill( (reco_jet_origE-true_jet_e)/true_jet_e, file_weight);    
    h_emfrac_origRESTruthJet->Fill( true_jet_emfrac, (reco_jet_origE-true_jet_e)/true_jet_e, file_weight);
    h_emfrac_origJESTruthJet->Fill( true_jet_emfrac, reco_jet_origE/true_jet_e, file_weight);
  ...
  //gMinuit->mnparm(1, "ihcal_scale", vstart[1], step[1], 1,10,ierflg);
  //gMinuit->mnparm(2, "ohcal_scale", vstart[2], step[2], 1,10,ierflg);
  static Double_t vstart[3] = {1.5, 2.5, 1.5}; //staring values
  static Double_t step[3] = {0.01, 0.01, 0.01}; //step
  gMinuit->mnparm(0, "cemc_scale", vstart[0], step[0], 1.001,5.0,ierflg);
  gMinuit->mnparm(1, "ihcal_scale", vstart[1], step[1], 1.05,3.1,ierflg);
  gMinuit->mnparm(2, "ohcal_scale", vstart[2], step[2], 1.001,5.0,ierflg);
  ...
  for (int ievt=0; ievt <nevt; ievt++){
    ...
    h_RESTruthJet->Fill( (reco_jet_caliE-true_jet_e)/true_jet_e, file_weight );    
    h_emfrac_RESTruthJet->Fill( true_jet_emfrac, (reco_jet_caliE-true_jet_e)/true_jet_e, file_weight );
    h_emfrac_JESTruthJet->Fill( true_jet_emfrac, reco_jet_caliE/true_jet_e, file_weight );
  ...
  cout << "h_origRESTruthJet->GetEntries() = " << h_origRESTruthJet->GetEntries() << endl;
  cout << "h_origRESTruthJet->Integral() = " << h_origRESTruthJet->Integral() << endl;
  c1->cd(2);
  h_RESTruthJet->Draw("pe");
  cout << "h_RESTruthJet->GetEntries() = " << h_RESTruthJet->GetEntries() << endl;
  cout << "h_RESTruthJet->Integral() = " << h_RESTruthJet->Integral() << endl;
  c1->cd(3);
  ...
  prof_emfrac_origRESTruthJet->SetMarkerColor(kPink+1);
  prof_emfrac_origRESTruthJet->SetLineColor(kPink+1);
  ...
  gStyle->SetOptFit(0);
  //gStyle->SetOptFit(1);
  prof_emfrac_RESTruthJet->SetMarkerColor(kPink+1);
  prof_emfrac_RESTruthJet->SetLineColor(kPink+1);
  ...
  c1->SaveAs(Form("minuit_truthjet_py8jet/%d_%d_%s_%s_%s_%s.pdf",initfile,endfile,sampleType.c_str(),jetE.c_str(),ihcalType.c_str(),note.c_str()));
  ...
  gStyle->SetOptFit(0);
  //gStyle->SetOptFit(1);
  prof_emfrac_origJESTruthJet->SetMarkerColor(kPink+1);
  prof_emfrac_origJESTruthJet->SetLineColor(kPink+1);
  ...
  gStyle->SetOptFit(0);
  //gStyle->SetOptFit(1);
  prof_emfrac_JESTruthJet->SetMarkerColor(kPink+1);
  prof_emfrac_JESTruthJet->SetLineColor(kPink+1);
  ...
  c2->SaveAs(Form("minuit_truthjet_py8jet/%d_%d_%s_%s_%s_%s_v2.pdf",initfile,endfile,sampleType.c_str(),jetE.c_str(),ihcalType.c_str(),note.c_str()));
 
  cout << "cemcScale = " << cemcScale << endl;
  cout << "ihcalScale = " << ihcalScale << endl;
  cout << "ohcalScale = " << ohcalScale << endl;
  //cout << "cemcEmScale = 1.00" << endl;

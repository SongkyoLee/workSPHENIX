...
void recursiveFit(TH1D* t_h1, TF1* t_tf1, float t_fitmin, float t_fitmax, float* t_mean, float* t_meanErr, float* t_sig, float* t_sigErr);

void final_qcdjet_RecoToTrue_py8jet(string sampleType = "py8Jet",
                              string ihcalType = "Al",
                              string jetE = "20GeV",
                              string note = "R04",
                              int initfile=0,
                              ...
                              bool dodphicut=true)
{
  ...
  double dummy;
  double cemc_diff[nEBin];
  double ihcal_diff[nEBin];
  double ohcal_diff[nEBin];

  string szMeth = "truthjet";
  //////////////////// read-in cemc files
  string szcemc;
  int icemc = 0;
  ifstream infile_cemc;
  infile_cemc.open(Form("txt_scale_py8jet/cemcScale_%s_%s_%s.out",szMeth.c_str(),ihcalType.c_str(),note.c_str()));
  icemc=0;
  if (infile_cemc.is_open()){
    while( getline (infile_cemc,szcemc) ) { cemc_diff[icemc] = atof(szcemc.c_str()); icemc++; }
  }
  //////////////////// read-in ihcal files
  string szihcal;
  int iihcal = 0;
  ifstream infile_ihcal;
  infile_ihcal.open(Form("txt_scale_py8jet/ihcalScale_%s_%s_%s.out",szMeth.c_str(),ihcalType.c_str(),note.c_str()));
  iihcal=0;
  if (infile_ihcal.is_open()){
    while( getline (infile_ihcal,szihcal) ) { ihcal_diff[iihcal] = atof(szihcal.c_str()); iihcal++; }
  }
  //////////////////// read-in ohcal files
  string szohcal;
  int iohcal = 0;
  ifstream infile_ohcal;
  infile_ohcal.open(Form("txt_scale_py8jet/ohcalScale_%s_%s_%s.out",szMeth.c_str(),ihcalType.c_str(),note.c_str()));
  iohcal=0;
  if (infile_ohcal.is_open()){
    while( getline (infile_ohcal,szohcal) ) { ohcal_diff[iohcal] = atof(szohcal.c_str()); iohcal++; }
  }
 
  for (int ie=0; ie<nEBin; ie++){
    if (cemc_sf==1) { cemc_diff[ie] = 1.; }
    if (ihcal_sf==1) { ihcal_diff[ie] = 1.; }
    if (ohcal_sf==1) { ohcal_diff[ie] = 1.; }
    cout << "cemc_diff["<<ie<<"] = " << cemc_diff[ie] << endl;
    cout << "ihcal_diff["<<ie<<"] = " << ihcal_diff[ie] << endl;
    cout << "ohcal_diff["<<ie<<"] = " << ohcal_diff[ie] << endl;
   }  
  ...
  //fname = Form("./2ndSortedRootFilesPtBinning/2ndSorted_%dto%d_%s_G4sPHENIX_jet4_%s_%s_%s_doetopcut%d_dodphicut%d.root",initfile,endfile,sampleType.c_str(),jetE.c_str(),ihcalType.c_str(),note.c_str(),(int)doetopcut,(int)dodphicut);
  //fname = Form("./2ndSortedRootFilesPtBinningMerged/2ndSorted_%dto%d_%s_G4sPHENIX_jet4_noGeV_%s_%s_doetopcut%d_dodphicut%d.root",initfile,endfile,sampleType.c_str(),ihcalType.c_str(),note.c_str(),(int)doetopcut,(int)dodphicut);
  fname = Form("./2ndSortedRootFilesPtBinningPy8jet/2ndSorted_%dto%d_%s_G4sPHENIX_jet4_noGeV_%s_%s_doetopcut%d_dodphicut%d.root",initfile,endfile,sampleType.c_str(),ihcalType.c_str(),note.c_str(),(int)doetopcut,(int)dodphicut);
  ...
  out_tree->SetBranchAddress("file_weight",&file_weight);
  ...
  for (int ievt=0; ievt <nevt; ievt++){
    ...
    if (jetE.compare("20GeV")==0) { if (!(true_jet_pt>20 && true_jet_pt<30)) continue; }
    else if (jetE.compare("30GeV")==0) { if (!(true_jet_pt>30 && true_jet_pt<40)) continue; }
    else if (jetE.compare("40GeV")==0) { if (!(true_jet_pt>40 && true_jet_pt<50)) continue; }
    else if (jetE.compare("50GeV")==0) { if (!(true_jet_pt>50 && true_jet_pt<60)) continue; }
    else { cout << "select proper jetE" << endl; }
    
    //if (reco_jet_pt>20 && reco_jet_pt<30) { cemc_sf = cemc_diff[0]; ihcal_sf = ihcal_diff[0]; ohcal_sf = ohcal_diff[0]; }
    if (reco_jet_pt<30) { cemc_sf = cemc_diff[0]; ihcal_sf = ihcal_diff[0]; ohcal_sf = ohcal_diff[0]; }
    else if (reco_jet_pt>30 && reco_jet_pt<40) { cemc_sf = cemc_diff[1]; ihcal_sf = ihcal_diff[1]; ohcal_sf = ohcal_diff[1]; }
    else if (reco_jet_pt>40) { cemc_sf = cemc_diff[2]; ihcal_sf = ihcal_diff[2]; ohcal_sf = ohcal_diff[2]; }
    //else if (reco_jet_pt>40 && reco_jet_pt<50) { cemc_sf = cemc_diff[2]; ihcal_sf = ihcal_diff[2]; ohcal_sf = ohcal_diff[2]; }
    //else if (reco_jet_pt>50) { cemc_sf = cemc_diff[3]; ihcal_sf = ihcal_diff[3]; ohcal_sf = ohcal_diff[3]; }
    //else if (reco_jet_pt>50 && reco_jet_pt<60) { cemc_sf = cemc_diff[3]; ihcal_sf = ihcal_diff[3]; ohcal_sf = ohcal_diff[3]; }
    else { cout << "select proper jetE" << endl; }
    
    //// scaling
    reco_jet_clcemc_emEsum = reco_jet_clcemc_emEsum*total_sf;
    reco_jet_clcemc_hadEsum = reco_jet_clcemc_hadEsum*cemc_sf*total_sf;
    reco_jet_ihcalEsum = reco_jet_ihcalEsum*ihcal_sf*total_sf;
    reco_jet_ohcalEsum = reco_jet_ohcalEsum*ohcal_sf*total_sf;
    ...
    h_IHAsymm -> Fill(ihasymm,ihfrac, file_weight); 
    h_EHAsymm -> Fill(ehasymm,ehfrac, file_weight); 
    h_totEHAsymm -> Fill(totehasymm,totehfrac, file_weight); 
    h_emfrac -> Fill(true_jet_emfrac,( reco_jet_clcemc_emEsum + reco_jet_clcemc_hadEsum + reco_jet_ihcalEsum + reco_jet_ohcalEsum )/true_jet_e, file_weight);
    
    h_JES->Fill(( reco_jet_clcemc_emEsum + reco_jet_clcemc_hadEsum + reco_jet_ihcalEsum + reco_jet_ohcalEsum )/true_jet_e, file_weight);
    //h_JER->Fill(( reco_jet_clcemc_emEsum + reco_jet_clcemc_hadEsum + reco_jet_ihcalEsum + reco_jet_ohcalEsum - true_jet_e)/ ( reco_jet_clcemc_emEsum + reco_jet_clcemc_hadEsum + reco_jet_ihcalEsum + reco_jet_ohcalEsum ), file_weight);
    h_JPTS->Fill(reco_jet_pt/true_jet_pt, file_weight);
    h_JMS->Fill(reco_jet_m/true_jet_m, file_weight);
  }
...
//  palette->SetX2NDC(1-margin*3);
  ...
  ////////////////////////////////////////
  if (total_sf !=1) {
    c1->SaveAs(Form("outFinal%sRecoToTruePy8jet/final2D_%s_%s_%s_%s_%.0f_%.0f_%.0f_wTot%.0f.pdf",ihcalType.c_str(),sampleType.c_str(),jetE.c_str(),ihcalType.c_str(),note.c_str(),100*cemc_sf,100*ihcal_sf,100*ohcal_sf,100*total_sf));
  } else {
    c1->SaveAs(Form("outFinal%sRecoToTruePy8jet/final2D_%s_%s_%s_%s_%.0f_%.0f_%.0f.pdf",ihcalType.c_str(),sampleType.c_str(),jetE.c_str(),ihcalType.c_str(),note.c_str(),100*cemc_sf,100*ihcal_sf,100*ohcal_sf));
  }
  ...
  recursiveFit(h_JES, tf_JES, fitmin_jes, fitmax_jes, &mean_jes, &meanErr_jes, &sig_jes, &sigErr_jes);
  ...
  recursiveFit(h_JPTS, tf_JPTS, fitmin_jpts, fitmax_jpts, &mean_jpts,  &meanErr_jpts, &sig_jpts, &sigErr_jpts);
  for (int ir=0; ir<repeat; ir++){
  ...
  recursiveFit(h_JMS, tf_JMS, fitmin_jms, fitmax_jms, &mean_jms, &meanErr_jms, &sig_jms, &sigErr_jms);
  for (int ir=0; ir<repeat; ir++){
...
/*
  ...
  recursiveFit(h_JER, tf_JER, fitmin_jer, fitmax_jer, &mean_jer, &meanErr_jer, sig_jer, &sigErr_jer);
  for (int ir=0; ir<repeat; ir++){
...
*/
  ...
  if (total_sf !=1) {
    c2->SaveAs(Form("outFinal%sRecoToTruePy8jet/final1D_%s_%s_%s_%s_%.0f_%.0f_%.0f_wTot%.0f.pdf",ihcalType.c_str(),sampleType.c_str(),jetE.c_str(),ihcalType.c_str(),note.c_str(),100*cemc_sf,100*ihcal_sf,100*ohcal_sf,100*total_sf));
  } else {
    c2->SaveAs(Form("outFinal%sRecoToTruePy8jet/final1D_%s_%s_%s_%s_%.0f_%.0f_%.0f.pdf",ihcalType.c_str(),sampleType.c_str(),jetE.c_str(),ihcalType.c_str(),note.c_str(),100*cemc_sf,100*ihcal_sf,100*ohcal_sf));
  }
  ...
  if (cemc_sf==1 && ihcal_sf==1 && ohcal_sf==1) { 
    fout = new TFile(Form("outFinal%sRecoToTruePy8jet/final_%s_%s_%s_%s_before.root",ihcalType.c_str(),sampleType.c_str(),jetE.c_str(),ihcalType.c_str(),note.c_str()),"RECREATE"); 
  } else { 
    if (total_sf!=1){
      fout = new TFile(Form("outFinal%sRecoToTruePy8jet/final_%s_%s_%s_%s_truthjet_wTot.root",ihcalType.c_str(),sampleType.c_str(),jetE.c_str(),ihcalType.c_str(),note.c_str()),"RECREATE"); //tmp 
    } else {
      fout = new TFile(Form("outFinal%sRecoToTruePy8jet/final_%s_%s_%s_%s_truthjet.root",ihcalType.c_str(),sampleType.c_str(),jetE.c_str(),ihcalType.c_str(),note.c_str()),"RECREATE"); //tmp
    }
  }
